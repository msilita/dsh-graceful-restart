...
**修复"闲置会话被唤醒后阻塞关闭/重启"**——上次完整重启测试暴露的真实问题：

- **问题**：`restart_harness` 用 `ctx.agents.roots()` 记录 marker——**所有活跃会话**
  都会被记录，包括闲置的旧测试会话（如 15:52 创建、测了一句话的会话）。重启后
  这些闲置会话也被唤醒，各自跑一整轮 LLM：既阻塞关闭/重启的等待，又白白消耗算力；
  更糟的是被唤醒的会话还能调用 `cancel_harness_action` **撤销本会话发起的关闭**。
- **唤醒范围收紧**：插件监听 `agent/status` + `session/event` 追踪每个会话的最近
  活动时间；marker 只记录**30 分钟内活跃过**的根会话（发起者必定包含）。闲置的
  旧会话不再被唤醒。
- **cancel 发起者校验**：`scheduleExit` 记录安排发起者（工具 execute 的
  `exec.agent.id`）；`cancelPendingExit` 只允许发起者撤销，其他会话的撤销请求被
  拒绝并记录日志（`cancel DENIED: caller X != owner Y`）。
- **等待进度增强**：关闭/重启等待时，终端每 5s 打印在等谁，并标注**超过 60s 无
  事件流**的会话（疑似卡住），便于用户判断。
- **绝不超时强制关闭**：显式等待所有轮次结束，不打断正在运行的工作；70s 兜底
  重置保留（打印提示，可再次触发）。
...
- 安排时立即打印（`关闭已安排：等当前轮次结束后进程退出...`）
- 等待时每 5s 打印进度（`关闭等待中：还有 1 个会话在运行（80c3d7f3）...`）
- 空闲/70s 超时重置/取消都有终端输出，不再有"静默"阶段
- **绝不超时强制关闭**：显式等所有轮次结束（曾考虑 45s 上限，会打断运行中的工作，已撤销）
