	...
	factory: (require) => {
		...
		function SettingsSection() {
			...
			for (var i = 0; i < history.length; i++) {
				var h = history[i];
				var cells = [];
				// 版本列：该版本的包数量（git 风格徽标）
				var pkgCount = Object.keys(h.deps || {}).length;
				if (h.diff) {
					// git 配色：+ 新增绿 / - 移除红 / ~ 版本变化黄；只显示包名，版本详情在 tooltip
					if (h.diff.added && h.diff.added.length) {
						for (var ai = 0; ai < h.diff.added.length; ai++) {
							var a = h.diff.added[ai];
							cells.push(jsx("div", { key: "a" + ai, title: "+ " + nameOf(a) + " " + verOf(a), style: { color: "#2e7d32", fontSize: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } },
								"+ " + nameOf(a)));
						}
					}
					if (h.diff.removed && h.diff.removed.length) {
						for (var ri = 0; ri < h.diff.removed.length; ri++) {
							...
							cells.push(jsx("div", { key: "r" + ri, title: "- " + nameOf(r) + " " + verOf(r), style: { color: "#c62828", fontSize: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } },
								"- " + nameOf(r)));
						}
					}
					if (h.diff.changed && h.diff.changed.length) {
						for (var ci = 0; ci < h.diff.changed.length; ci++) {
							...
							cells.push(jsx("div", { key: "c" + ci, title: "~ " + nameOf(c) + ": " + (c.from !== undefined ? c.from : "?") + " → " + (c.to !== undefined ? c.to : "?"), style: { color: "#b58900", fontSize: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } },
								"~ " + nameOf(c)));
						}
					}
					if (cells.length === 0) cells.push(jsx("div", { key: "n", style: { fontSize: 12, opacity: 0.6 } }, "（无变化）"));
				} else {
					cells.push(jsx("div", { key: "base", style: { fontSize: 12, opacity: 0.7 } }, "（完整基线）"));
				}
				if (h.note) cells.push(jsx("div", { key: "note", style: { fontSize: 12, opacity: 0.7 } }, "「" + h.note + "」"));
				histRows.push(jsx("tr", { key: "h" + i },
					jsx("td", { style: { ...tdStyle, width: 150, whiteSpace: "nowrap" } }, [
						fmtAt(h.at),
						// git 风格徽标：当前标记在时间右侧
						i === 0 && jsx("span", { style: { marginLeft: 6, padding: "1px 6px", borderRadius: 8, background: "#2e7d32", color: "#fff", fontSize: 10, fontWeight: 600 } }, "当前"),
					]),
					// 版本列：包数量徽标（窄列）
					jsx("td", { style: { ...tdStyle, width: 48, textAlign: "center" } },
						jsx("span", { style: { display: "inline-block", padding: "1px 8px", borderRadius: 8, border: "1px solid currentColor", fontSize: 11, opacity: 0.8 } }, pkgCount + " 包")),
					jsx("td", {
						// 错误版本标红 + tooltip 显示错误详情
					...
					jsx("table", { style: { width: "100%", borderCollapse: "collapse", fontSize: 12 } }, [
						jsx("thead", {},
							jsx("tr", {},
								...
								jsx("th", { style: { ...thStyle, width: 60, textAlign: "center" } }, "版本"),
