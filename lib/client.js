window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		exports.apply = function apply(ctx) {
			...
			// **确定性信号（事件驱动，无轮询、无超时）**：loader 的 fiber 状态变化
			// 通过 `internal/status` 事件广播（dsh-client-web 内核用同一事件驱动
			// 故障页渲染）。本插件 client 半（entry 未失败，apply 正常执行）监听
			// 该事件：任何 entry 进入非 active 终态（failed）→ boot 必然停留故障页
			// → 上报 host /page-failed → host 走统一故障链（回滚坏插件 → 重试）。
			// 不依赖页面文本（避免正常页内容误判）、不轮询、无时间窗口。
			var pageFailedReported = false;
			var failedEntryNames = [];
			var reportPageFailed = function () {
				if (pageFailedReported) return;
				pageFailedReported = true;
				var entries = failedEntryNames.join(", ");
				fetch("/plugins/dsh-graceful-restart/page-failed", {
					method: "POST",
					headers: { "content-type": "application/json" },
					body: JSON.stringify({ detail: "页面显示加载失败（Failed to load plugins）", failedEntries: entries }),
				}).catch(function () {});
			};
			// 监听 loader fiber 状态：entry 进入 failed 终态 → 故障页必然出现
			// 只认 "failed"（与内核 AppRoot 判定一致 s === "failed"）——
			// loading/pending 等中间态不报，避免把正常加载误判为故障。
			// 事件驱动：无轮询、无时间窗口；失败状态一到即上报。
			try {
				ctx.on("internal/status", function (fiber) {
					if (!fiber || !fiber.entry || pageFailedReported) return;
					if (fiber.state !== "failed") return;
					var name = fiber.entry.options && fiber.entry.options.name;
					if (name && name !== "dsh-graceful-restart") failedEntryNames.push(name);
					reportPageFailed();
				});
			} catch (e) {}
