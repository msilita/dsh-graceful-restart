window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SettingsSection() {
			...
			// 当前标记：标在与 current deps 匹配的完整基线条目上
			// （回滚后指针后退 → [当前] 在回退到的版本，如 11:00；失败尝试 ⚠ 在顶部）
			var curDeps = snap.current && snap.current.deps;
			var currentIdx = 0;
			if (curDeps) {
				for (var ci0 = 0; ci0 < history.length; ci0++) {
					if (history[ci0].deps && JSON.stringify(history[ci0].deps) === JSON.stringify(curDeps)) {
						currentIdx = ci0;
						break;
					}
				}
			...
			for (var i = 0; i < history.length; i++) {
				...
				histRows.push(jsx("tr", { key: "h" + i },
					jsx("td", { style: { ...tdStyle, width: 150, whiteSpace: "nowrap" } }, [
						...
						h.error && jsx("span", { style: { marginLeft: 6, color: "#c62828", fontSize: 13 } }, "⚠"),
					]),
					jsx("td", {
			...
			return jsx("div", { style: { padding: 4 } }, [
				...
				jsx("div", { style: rowStyle }, [
					...
					// 错误信息：表格下方完整显示（两级：概要在上，具体报错缩进在下；不弹窗）
					(() => {
						var errs = history.filter(function (h) { return h.error; });
						if (errs.length === 0) return null;
						return jsx("div", { style: { marginTop: 8, padding: 8, borderRadius: 6, background: "rgba(198,40,40,0.08)", border: "1px solid rgba(198,40,40,0.3)" } },
							errs.map(function (h, ei) {
								return jsx("div", { key: "err" + ei, style: { fontSize: 12, color: "#c62828", marginBottom: ei < errs.length - 1 ? 8 : 0 } }, [
									jsx("div", { style: { whiteSpace: "pre-wrap", wordBreak: "break-all" } }, "⚠ " + fmtAt(h.error.at) + " " + (h.error.detail || "")),
									h.error.stderr ? jsx("pre", { style: { margin: "4px 0 0", padding: "6px 8px", borderRadius: 4, background: "rgba(0,0,0,0.35)", color: "#e0b3b3", fontSize: 11, whiteSpace: "pre-wrap", wordBreak: "break-all" } }, h.error.stderr) : null,
								]);
							}),
						);
					})(),
