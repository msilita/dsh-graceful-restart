window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SettingsSection() {
			...
			// 错误弹窗：点击 ⚠ 打开（完整错误 + 复制按钮）
			var modal = React.useState(null);
			var modalErr = modal[0];
			var setModalErr = modal[1];
			var copied = React.useState(false);
			var copiedFlag = copied[0];
			var setCopiedFlag = copied[1];

			function copyError() {
				if (!modalErr) return;
				try {
					if (navigator.clipboard && navigator.clipboard.writeText) {
						navigator.clipboard.writeText(modalErr.detail || "").then(function () { setCopiedFlag(true); });
					} else {
						var ta = document.createElement("textarea");
						ta.value = modalErr.detail || "";
			...
			// 当前标记：第一条若无 error（正常最新版本）→ 标第一条；
			// 第一条有 error（失败尝试保留在顶部）→ 当前是第二条
			var currentIdx = history.length > 0 && history[0].error ? 1 : 0;
			...
			for (var i = 0; i < history.length; i++) {
				...
				if (h.diff) {
					...
					if (h.diff.changed && h.diff.changed.length) {
						for (var ci = 0; ci < h.diff.changed.length; ci++) {
							...
							rows.push({
								text: "~ " + nameOf(c) + "@" + (c.to !== undefined ? c.to : "?") + "（旧 " + (c.from !== undefined ? c.from : "?") + "）",
								color: "#b58900",
							});
						}
					}
				...
				histRows.push(jsx("tr", { key: "h" + i },
					jsx("td", { style: { ...tdStyle, width: 150, whiteSpace: "nowrap" } }, [
						...
						h.error && jsx("span", {
							onClick: function () { setModalErr(h.error); setCopiedFlag(false); },
							style: { marginLeft: 6, color: "#c62828", cursor: "pointer", fontSize: 13 },
						}, "⚠"),
					]),
					jsx("td", {
						style: h.error ? { ...tdStyle, backgroundColor: "rgba(198,40,40,0.12)" } : tdStyle,
						title: h.error ? (h.error.detail || "") : undefined,
					},
						...
						}).concat(h.note ? [jsx("div", { key: "note", style: { fontSize: 11, opacity: 0.7 } }, "「" + h.note + "」")] : []),
					),
				));
			}
			...
			return jsx("div", { style: { padding: 4 } }, [
				...
				jsx("div", { style: rowStyle }, [
					jsx("label", { style: labelStyle }, "回滚上限（自动保存）"),
					jsx("input", {
						type: "number", min: 1, max: 10,
						style: inputStyle,
						value: value.rollbackLimit == null ? 2 : value.rollbackLimit,
						onChange: function (e) { autoSave({ rollbackLimit: Number(e.target.value) }); },
					}),
					jsx("span", { style: descStyle }, "失败时向更早基线逐级回滚，最多尝试 1-10 次（默认 2）后放弃；历史基线无限保留。" + (state.saved ? " ✓ 已保存" : "") + (state.error ? " ✗ " + String(state.error) : "")),
				]),
				jsx("div", { style: rowStyle }, [
				...
				modalErr && jsx("div", {
					...
					onClick: function () { setModalErr(null); },
				},
					jsx("div", {
						style: { background: "#fff", color: "#222", borderRadius: 8, padding: 16, maxWidth: 520, width: "92%", boxSizing: "border-box", boxShadow: "0 8px 32px rgba(0,0,0,0.3)" },
						onClick: function (e) { e.stopPropagation(); },
					}, [
						jsx("div", { style: { fontWeight: 600, marginBottom: 8 } }, "错误详情（" + fmtAt(modalErr.at) + "）"),
						jsx("pre", { style: { whiteSpace: "pre-wrap", wordBreak: "break-all", fontSize: 12, maxHeight: 260, overflow: "auto", margin: 0, background: "#f5f5f5", padding: 10, borderRadius: 6 } }, modalErr.detail || ""),
						jsx("div", { style: { display: "flex", gap: 8, marginTop: 12, justifyContent: "flex-end" } }, [
							jsx("button", { onClick: copyError, style: buttonStyle }, copiedFlag ? "已复制 ✓" : "复制"),
							jsx("button", { onClick: function () { setModalErr(null); }, style: buttonStyle }, "关闭"),
						]),
					]),
				),
			]);
