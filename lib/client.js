window.__ModuleLoader__.load({
	id: "dsh-graceful-restart",
	factory: (require) => {
		var module = { exports: {} };
		var exports = module.exports;
		Object.defineProperty(exports, Symbol.toStringTag, { value: "Module" });
		/**
		 * dsh-graceful-restart — client 半。
		 *
		 * 无任何提示 UI（完全无感）：只负责"重启后自动刷新"。
		 *
		 * 机制（pid 对比，覆盖所有连接情况）：
		 *   · 页面加载时记录"当前进程 pid"（loadedPid）
		 *   · 每次 connection/reset 后查 /status（失败带退避重试，覆盖守护回滚的服务真空期）：
		 *       - pid ≠ loadedPid → 页面存活期间进程重启过（wrapper 重启 / 手动完整重启 /
		 *         守护回滚）→ 自动刷新一次
		 *       - pid 相同 → 无动作（ws 抖动/重连，页面本身已是最新）
		 *   · 刷新后的新页面 loadedPid 匹配当前进程 → 不再刷新（防循环）
		 */
		exports.name = "dsh-graceful-restart-client";
		exports.apply = function apply(ctx) {
			// 每次页面加载都发 ack（幂等；Host 未就绪时失败静默）
			try {
				fetch("/plugins/dsh-graceful-restart/ack", { method: "POST" }).catch(function () {});
			} catch (e) {}

			// 页面加载时记录"当前进程 pid"：判断"连接到的进程是否与页面加载时一致"。
			var loadedPidPromise = fetch("/plugins/dsh-graceful-restart/status", { method: "GET" })
				.then(function (res) { return res.json().catch(function () { return null; }); })
				.then(function (data) { return data && data.pid != null ? String(data.pid) : null; })
				.catch(function () { return null; });

			// 查 /status：守护回滚时存在"服务真空期"（旧进程已死、新进程未起，
			// 中间要执行 remove 插件 + 重新 spawn），fetch 会失败——带退避重试，
			// 覆盖真空期（20 次 × 500ms ≈ 10s 窗口）
			function queryStatus(attemptsLeft) {
				return fetch("/plugins/dsh-graceful-restart/status", { method: "GET" })
					.then(function (res) { return res.json().catch(function () { return null; }); })
					.catch(function () {
						if (attemptsLeft <= 0) return null;
						return new Promise(function (resolve) {
							window.setTimeout(function () {
								queryStatus(attemptsLeft - 1).then(resolve);
							}, 500);
						});
					});
			}

			// 监听连接建立/恢复事件：确认 restarted 信号后决定是否刷新
			var timer = null;
			var reloading = false;
			ctx.on("connection/reset", function () {
				if (timer !== null) return; // 同一次连接只确认一次
				timer = window.setTimeout(function () { timer = null; }, 1000);
				try {
					loadedPidPromise.then(function (loadedPid) {
						return queryStatus(20).then(function (data) {
							// 判定只看 pid 对比：页面存活期间进程变了（wrapper 重启 / 手动完整
							// 重启 / 守护回滚）→ 刷新；pid 不变（ws 抖动/重连）→ 不刷新。
							// 不依赖 restarted 标志——手动 `dsh web` 重启时该标志为 false。
							if (data && data.pid != null) {
								var pid = String(data.pid);
								if (loadedPid !== null && pid !== loadedPid && !reloading) {
									// 页面存活期间进程变了（重启/回滚）→ 刷新一次（新页面 loadedPid 会匹配，不再刷新）
									reloading = true;
									window.location.reload();
								}
							}
						});
					}).catch(function () {});
				} catch (e) {}
			});
		};
		return module.exports;
	}
});
