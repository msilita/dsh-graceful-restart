window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		exports.apply = function apply(ctx) {
			...
			/* ------------------------------------------------------------------ */
			/* 故障页检测：刷新后是"故障页"还是"正常页"的判断通道                      */
			/* ------------------------------------------------------------------ */
			// DSH boot 失败时（任一 client entry 加载失败），AppRoot 停留在
			// "Failed to load plugins" 错误页（fail loud, no partial UI）。
			// **确定性信号**：loader 加载 entry 失败会向浏览器 console 输出真实错误
			// （如 `exports is not defined` / `did not activate`）——本插件 client 半
			// （entry 未失败，apply 正常执行）**拦截 console.error + window.onerror**，
			// 捕获 loader 错误特征（真实错误输出，非页面内容——不会误报正常页），
			// 上报 host /page-failed → host 走统一故障链（回滚坏插件 → 重试）。
			// 事件驱动：无轮询、无超时。
			var pageFailedReported = false;
			var reportPageFailed = function (detail) {
				if (pageFailedReported) return;
				pageFailedReported = true;
				fetch("/plugins/dsh-graceful-restart/page-failed", {
					method: "POST",
					headers: { "content-type": "application/json" },
					body: JSON.stringify({ detail: detail || "页面显示加载失败（Failed to load plugins）" }),
				}).catch(function () {});
			};
			// loader 错误特征：真实错误输出中的关键模式（只在加载失败时出现）
			var isLoaderError = function (text) {
				return /did not activate|loader entry|Failed to load plugins|exports is not defined|Cannot use 'import'|failed to import/i.test(text);
			};
			...
			try {
				...
				console.error = function () {
					try {
						...
						if (isLoaderError(text)) reportPageFailed(text.slice(0, 300));
					} catch (e) {}
					return origConsoleError.apply(console, arguments);
				};
			} catch (e) {}
			// 2) window.onerror + unhandledrejection（未捕获异常/动态 import 兜底）
			try {
				window.addEventListener("error", function (ev) {
					try {
						var text = String((ev && (ev.message || (ev.error && ev.error.message))) || "");
						if (isLoaderError(text)) reportPageFailed(text.slice(0, 300));
					} catch (e) {}
				});
				window.addEventListener("unhandledrejection", function (ev) {
					try {
						var r = ev && ev.reason;
						var text = String((r && (r.message || r.reason)) || r || "");
						if (isLoaderError(text)) reportPageFailed(text.slice(0, 300));
					} catch (e) {}
				});
			} catch (e) {}
			// 3) 页面 boot 后的确定性检测（事件驱动）：DSH boot 完成时 DOM 会切换
			//    到真实 UI；故障页停留。用 MutationObserver 观察 AppRoot 容器：
			//    出现 "Failed to load plugins" 标题（内核硬编码文本，稳定信号）时上报。
			//    仅检测内核渲染的失败标题（结构性位置），不匹配页面内容。
			try {
				var failTitleSeen = false;
				var observer = new MutationObserver(function () {
					if (failTitleSeen) return;
					// AppRoot 失败页的标题文本（内核硬编码 "Failed to load plugins"，
					// 位于 boot 卡片内，是结构性信号；页面内容里的同名文本不在该容器）
					var all = document.querySelectorAll("div");
					for (var i = 0; i < all.length; i++) {
						var el = all[i];
						if (el.childElementCount === 0 && el.textContent === "Failed to load plugins") {
							failTitleSeen = true;
							reportPageFailed("页面显示加载失败（Failed to load plugins）");
							return;
						}
					}
				});
