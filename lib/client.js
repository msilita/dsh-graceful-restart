	...
	factory: (require) => {
		...
		function SettingsSection() {
			...
			for (var i = 0; i < history.length; i++) {
				...
				var cells = [];
				if (h.diff) {
					// git 配色：新增绿 +、移除红 -、版本变化黄 ~（包名+版本）
					if (h.diff.added && h.diff.added.length) {
						for (var ai = 0; ai < h.diff.added.length; ai++) {
							var a = h.diff.added[ai];
							cells.push(jsx("div", { key: "a" + ai, style: { color: "#2e7d32", fontSize: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } },
								"+ " + nameOf(a) + verOf(a)));
						}
					}
					if (h.diff.removed && h.diff.removed.length) {
						for (var ri = 0; ri < h.diff.removed.length; ri++) {
							...
							cells.push(jsx("div", { key: "r" + ri, style: { color: "#c62828", fontSize: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } },
								"- " + nameOf(r) + verOf(r)));
						}
					}
					if (h.diff.changed && h.diff.changed.length) {
						for (var ci = 0; ci < h.diff.changed.length; ci++) {
							...
							cells.push(jsx("div", { key: "c" + ci, style: { color: "#b58900", fontSize: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } },
								"~ " + nameOf(c) + ": " + (c.from !== undefined ? c.from : "?") + " → " + (c.to !== undefined ? c.to : "?")));
						}
					}
