window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		exports.apply = function apply(ctx) {
			...
			/* ------------------------------------------------------------------ */
			/* 页面横幅：加载失败/回滚等故障在页面可见（页面模式下终端不可见）         */
			/* ------------------------------------------------------------------ */
			// 读取快照（errors/rollbackLog/刷新失败等），非空时在页面顶部显示警示条。
			// 轮询间隔 5s；横幅内嵌"关闭"按钮可手动隐藏到本次会话。
			var bannerDismissed = false;
			var bannerCheck = function () {
				...
				fetch("/plugins/dsh-graceful-restart/snapshot", { method: "GET" })
					...
					.then(function (data) {
						...
						for (var i = 0; i < errs.length; i++) {
							if (errs[i] && errs[i].detail) items.push("⚠ " + errs[i].detail);
						}
						// 最近的唤醒/回滚过程（若开始于最近 2 分钟内）——正在恢复中提示
						var recent = Date.now() - 2 * 60 * 1000;
						var fmt = function (iso) { try { return new Date(iso).toLocaleString("zh-CN", { hour12: false }); } catch (e) { return iso || ""; } };
						if (data.rollbackLog && data.rollbackLog.startedAt && new Date(data.rollbackLog.startedAt).getTime() > recent) {
							items.push("🔄 最近回滚：" + fmt(data.rollbackLog.startedAt) + " 起 " + (data.rollbackLog.entries || []).length + " 步");
						}
						if (data.wakeLog && data.wakeLog.startedAt && new Date(data.wakeLog.startedAt).getTime() > recent) {
							items.push("⏰ 最近唤醒：" + fmt(data.wakeLog.startedAt));
						}
						// 若刷新失败出现在 errors 里（refresh-failed 记录），横幅给出明确指引
						var refreshFailed = errs.some(function (e) { return e.detail && e.detail.indexOf("刷新阶段自检失败") === 0; });
						if (refreshFailed) items.push("🛑 刷新阶段自检失败——系统将自动回滚重试；若持续失败请查看设置页「优雅重启」的详细错误");

						if (items.length === 0) return;
						var banner = document.createElement("div");
						banner.id = "dsh-graceful-restart-banner";
						banner.style.cssText = "position:fixed;top:0;left:0;right:0;z-index:2147483647;padding:10px 44px 10px 14px;background:#c62828;color:#fff;font-size:13px;line-height:1.5;box-shadow:0 2px 8px rgba(0,0,0,.4);font-family:system-ui,sans-serif;";
						var close = document.createElement("button");
						close.textContent = "✕";
						close.style.cssText = "position:absolute;top:6px;right:10px;background:transparent;border:none;color:#fff;font-size:16px;cursor:pointer;";
						close.onclick = function () { bannerDismissed = true; if (banner.parentNode) banner.parentNode.removeChild(banner); };
						...
						// 防重复：已存在则不重复插入
						if (!document.getElementById("dsh-graceful-restart-banner")) {
			...
			try {
				window.setTimeout(bannerCheck, 1500);
				window.setInterval(bannerCheck, 5000);
			} catch (e) { console.warn("[dsh-graceful-restart] banner check failed:", e); }
