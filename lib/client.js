	...
	factory: (require) => {
		...
		function SettingsSection() {
			...
			for (var i = 0; i < history.length; i++) {
				...
				if (h.note) changes = changes + "  「" + h.note + "」";
			...
			return jsx("div", { style: { padding: 4 } }, [
				...
				snap.current && snap.current.error && jsx("div", { style: rowStyle }, [
					jsx("label", { style: labelStyle }, "当前状态错误（HEAD）"),
					jsx("div", { style: { fontSize: 12, color: "#c62828", whiteSpace: "pre-wrap" } },
						"· " + fmtAt(snap.current.error.at) + " " + snap.current.error.detail),
				]),
