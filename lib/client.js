window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		exports.apply = function apply(ctx) {
			...
			// 但各 entry 的 fiber 独立——本插件 client 半（entry 未失败）仍会 apply，
			// 可以轮询 DOM 检测故障页特征文本。检测到 → 上报 host /page-failed →
			// host 走统一故障链（refresh-failed → 回滚坏插件 → 重试）。
			// 仅检测、不上报 UI（无横幅）；正常页无此文本，不误报。
			var pageFailedReported = false;
			var detectFailedPage = function () {
				if (pageFailedReported) return;
				try {
					var body = document.body ? document.body.innerText : "";
					if (body.indexOf("Failed to load plugins") === -1) return; // 正常页
					// 故障页：收集失败 entry 信息（AppRoot 列出 failed item 文本）
					var entries = "";
					var failedItems = document.querySelectorAll('[class*="failed"]');
					for (var i = 0; i < failedItems.length; i++) {
						var t = (failedItems[i].innerText || "").trim();
						if (t && t.indexOf("Failed to load plugins") === -1) {
							entries += (entries ? ", " : "") + t.split("\n")[0].slice(0, 60);
						}
					}
			...
			try {
				window.setTimeout(detectFailedPage, 2000);   // 页面加载后稍候（boot 判定完成）
				window.setInterval(detectFailedPage, 3000);  // 持续检测（故障页停留期间）
			} catch (e) {}
