	...
	factory: (require) => {
		...
		function SettingsSection() {
			...
			for (var i = 0; i < history.length; i++) {
				...
				if (h.diff) {
					...
					if (h.diff.changed && h.diff.changed.length) {
						for (var ci = 0; ci < h.diff.changed.length; ci++) {
							...
							rows.push({
								text: "~ " + nameOf(c) + " " + (c.from !== undefined ? c.from : "?") + " → " + (c.to !== undefined ? c.to : "?"),
								color: "#b58900",
							});
						}
					}
				...
				// git log 风格块：标题（时间 + 当前/错误标记）+ diff 行
				histBlocks.push(jsx("div", { key: "h" + i, style: { padding: "6px 0", borderBottom: "1px solid rgba(128,128,128,0.2)" } }, [
					jsx("div", { style: { fontSize: 12, fontWeight: 600, marginBottom: 2 } }, [
						fmtAt(h.at),
						i === 0 && jsx("span", { style: { marginLeft: 6, padding: "1px 6px", borderRadius: 8, background: "#2e7d32", color: "#fff", fontSize: 10, fontWeight: 600 } }, "当前"),
						h.error && jsx("span", { title: h.error.detail || "", style: { marginLeft: 6, color: "#c62828", cursor: "help" } }, "⚠"),
					]),
					jsx("div", { style: { fontFamily: "monospace", fontSize: 12, lineHeight: 1.5 } },
						rows.map(function (rw, ri2) {
							return jsx("div", { key: "r" + ri2, style: { color: rw.color || "inherit", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, rw.text);
						}),
					),
					h.note && jsx("div", { style: { fontSize: 11, opacity: 0.7 } }, "「" + h.note + "」"),
				]));
			}
			...
			return jsx("div", { style: { padding: 4 } }, [
				...
				jsx("div", { style: rowStyle }, [
					...
					jsx("div", { style: { padding: "4px 0" } }, histBlocks),
				]),
