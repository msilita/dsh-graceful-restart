				...
				if (h.diff) {
					...
					if (h.diff.changed && h.diff.changed.length) {
						for (var ci = 0; ci < h.diff.changed.length; ci++) {
							...
							rows.push({
								text: "~ " + nameOf(c),
								version: (c.from !== undefined ? c.from : "?") + " → " + (c.to !== undefined ? c.to : "?"),
								color: "#b58900",
							});
						}
					}
					if (rows.length === 0) rows.push({ text: "（无变化）", version: "", color: null });
				} else {
					rows.push({ text: "（完整基线）", version: "", color: null });
				}
				var cells = [];
				for (var ri2 = 0; ri2 < rows.length; ri2++) {
					var rw = rows[ri2];
					cells.push(jsx("div", { key: "r" + ri2, style: { display: "flex", gap: 8, alignItems: "baseline" } }, [
						jsx("span", { style: { flex: "0 0 auto", color: rw.color, fontSize: 12, whiteSpace: "nowrap" } }, rw.text),
						jsx("span", { style: { flex: "1 1 auto", fontSize: 11, opacity: 0.7, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, rw.version),
					]));
				}
