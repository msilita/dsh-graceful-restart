window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SettingsSection() {
			...
			// 键序无关比较（避免 JSON.stringify 因对象键序不同误判）
			var depsEqual = function (a, b) {
				if (!a || !b) return false;
				var ka = Object.keys(a), kb = Object.keys(b);
				if (ka.length !== kb.length) return false;
				for (var kd = 0; kd < ka.length; kd++) {
					if (!(ka[kd] in b) || b[ka[kd]] !== a[ka[kd]]) return false;
				}
				return true;
			};
			...
			if (curDeps) {
				for (var ci0 = 0; ci0 < history.length; ci0++) {
					if (history[ci0].deps && depsEqual(history[ci0].deps, curDeps)) {
						currentIdx = ci0;
						break;
					}
				}
