...
async function wrapperApply(ctx) {
  ...
  // 注：第一代不拆树、不卸载任何插件——拆根会触发 include 写回/重应用，
  // 实测影响第二代（含 bundle 插件）的加载，本末倒置。第一代保持完整加载
  // （官方 core 闲置无副作用；业务插件的双跑问题由插件自身克制或配置层解决）。
}
...
export async function apply(ctx) {
  ...
  log('apply: complete')
  // 注：自愈（快照驱动重建）已移除——重建 bundle 条目会破坏其 patch/加载语义，
   // 且插件正常时本就不需要；插件加载问题应在配置层排查，而非运行时重建。
}
