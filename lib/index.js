...
// 统一推送：把自定义状态事件追加到所有活跃会话日志流（事件名
// 'dsh-graceful-restart/status'）——已订阅 session/event 的客户端自动收到。

export const name = 'dsh-graceful-restart'
export const inject = ['commands', 'agents', 'timer', 'webServer', 'sessions']
...
/**
 * 统一推送：把状态事件追加到**触发重启/关闭的那个会话**的日志流
 * （不区分客户端——QQ/Web/TUI 订阅同一会话的 session/event 自动收到，
 * 客户端只需对未知事件类型做泛化转发，无需逐个接入）。
 * sessionId 为空（如纯命令触发、无发起会话）→ 不推送。
 */
function pushStatus(ctx, phase, detail, sessionId) {
  try {
    if (!sessionId) {
      log('pushStatus: ' + phase + ' skipped (no owner session)')
      return
    }
    const data = { phase, at: Date.now(), ...(detail ? { detail } : {}) }
    const s = ctx.sessions.list().find((x) => x.id === sessionId)
    if (!s) {
      log('pushStatus: ' + phase + ' skipped (session not live: ' + sessionId + ')')
      return
    }
    s.append('dsh-graceful-restart/status', data)
    log('pushStatus: ' + phase + (detail ? ' ' + detail : '') + ' → ' + sessionId)
    // 延迟修复历史：等持久化批量队列落盘后，给刚 append 的 status 事件补 ignorable
    // 标记——历史文件任何时刻都是干净的（卸载插件/其他 harness 读取也不拒）。
    // 幂等且仅处理 status 行，无 status 的文件秒退。
    try { setTimeout(() => { repairSessionHistory() }, 5000) } catch (e) { log('pushStatus: delayed repair schedule THREW ' + String(e)) }
  } catch (e) { log('pushStatus THREW ' + String(e)) }
}
...
/* 会话历史修复：给已写入的 dsh-graceful-restart/status 事件补 ignorable: true。
 * 背景：持久化层读取会话历史时对未知事件类型做严格校验——类型不在 KNOWN 集合
 * 且未标记 ignorable 的会被拒绝，导致**整段会话历史不可读**（曾出现 "unknown to
 * this harness and not marked ignorable" 错误，用户只能另起对话）。自定义类型
 * 写入历史无法避免（append 是唯一写入路径且不透传 ignorable），所以每次启动时
 * 主动修复历史文件：给 status 事件补上标记后，任何版本 harness 都能读取
 * （ignorable 事件被容忍跳过，不参与重构）。
 * 幂等：已带标记/无 status 事件的文件跳过；解码失败的帧放弃修复该文件（不写坏）。
 */
function repairSessionHistory() {
  try {
    ...
    const walk = (dir) => {
      ...
      for (const ent of ents) {
        ...
        else if (ent.name.endsWith('.jsonl.zstd') && !ent.name.includes('.bak')) {
          scanned++
          if (repairOneSessionFile(p)) fixed++
        }
      }
    }
    walk(sessionsDir)
    log('history repair: scanned ' + scanned + ' file(s), fixed ' + fixed)
    return fixed
  } catch (e) { log('history repair THREW ' + String(e)); return 0 }
}

function repairOneSessionFile(file) {
  try {
    const buf = fs.readFileSync(file)
    // 逐帧解码：zstd 帧自描述，从每个 magic 位置解码 subarray(i)（后续帧字节被忽略）
    const frames = []
    ...
    while (i < buf.length - 4) {
      if (buf[i] === 0x28 && buf[i + 1] === 0xB5 && buf[i + 2] === 0x2F && buf[i + 3] === 0xFD) {
        let dec
        try { dec = zstdDecompressSync(buf.subarray(i)) } catch { /* 假 magic 或损坏帧 */ }
        if (dec !== undefined) {
          const text = dec.toString('utf8')
          frames.push(text)
          if (!needsRepair && text.includes('"dsh-graceful-restart/status"')) {
            // 逐行精确检查（帧内可能混合已标记/未标记行）
            for (const line of text.split('\n')) {
              const t = line.trim()
              if (t.includes('"dsh-graceful-restart/status"') && !t.includes('"ignorable"')) { needsRepair = true; break }
            }
          }
        }
      }
    ...
    // 重建：status 事件行补 ignorable，其他行保持原文
    const out = frames.map((text) => {
      const lines = text.split('\n')
      for (let k = 0; k < lines.length; k++) {
        const t = lines[k].trim()
        if (!t || !t.includes('"dsh-graceful-restart/status"') || t.includes('"ignorable"')) continue
        try {
          const j = JSON.parse(t)
          if (j && j.type === 'dsh-graceful-restart/status' && j.ignorable !== true) {
            j.ignorable = true
            lines[k] = JSON.stringify(j)
          }
        } catch { /* 原样保留 */ }
      }
      return zstdCompressSync(Buffer.from(lines.join('\n'), 'utf8'))
    })
    // 原子写回：.tmp → rename 覆盖；首次修复前备份一份
    const tmp = file + '.repair-tmp'
    const bak = file + '.bak-repair'
    fs.writeFileSync(tmp, Buffer.concat(out))
    if (!fs.existsSync(bak)) fs.copyFileSync(file, bak)
    fs.renameSync(tmp, file)
    log('history repair: fixed ' + file + ' (' + frames.length + ' frames)')
    return true
  } catch (e) {
    log('history repair: file FAILED ' + file + ': ' + String(e))
    return false
  }
}
...
function scheduleExit(ctx, action, ownerId) {
  ...
  console.log('[dsh-graceful-restart] ' + actionLabel + '已安排：等当前轮次结束后'
    ...
    + '等待期间终端将显示进度。')
  // 统一推送"即将重启/关闭"（只追加到触发会话的日志流，客户端自动收到）
  try {
    if (action === 'shutdown') pushStatus(ctx, 'shutting-down', 'DSH 正在关闭', pendingActionOwner)
    else pushStatus(ctx, 'restarting', 'DSH 正在重启', pendingActionOwner)
  } catch (e) { log('pushStatus restarting THREW ' + String(e)) }
...
function tryResumeSessions(ctx, sessionIds, text, onResult) {
  ...
  // 无超时判断：持续轮询直到所有目标会话恢复并 steer（会话恢复是必然趋势，
  // 不设时间上限——避免"恢复慢的正常系统被超时误判为故障"）。
  // 失败判定交给确定性信号（client 半故障页上报 / boot 输出扫描）。
  ...
  const spawnChild = (wake, rollbackRetry = false) => {
    ...
    child.on('message', (msg) => {
      ...
      if (msg.type === 'restart') {
        ...
        try { child.kill() } catch {}
        // 竞态修复：child 可能已先退出（exit handler 已执行过，不会再次触发）——
        // 此时直接走 spawnChild，否则第一代将残留（曾导致 3081 被占、重启失败）
        if (child && child.exitCode !== null) {
          log('wrapper: child already exited (exitCode=' + child.exitCode + '), spawning directly')
          restarting = false
        ...
        deliberateExit = true
        try { child.kill() } catch {}
        // 竞态修复：child 可能已先退出（exit handler 已执行过）——
        // 直接退出，不依赖 exit handler 触发 process.exit（曾导致第一代残留占 3081）
        if (child && child.exitCode !== null) {
          log('wrapper: child already exited, exiting directly')
          process.exit(0)
          return
        }
...
export function apply(ctx) {
  ...
  // 会话历史修复：启动早期执行（早于任何历史读取），给已写入的 status 事件补
  // ignorable 标记，保证任何版本 harness 都能读取历史（幂等，见 repairSessionHistory）
  try { repairSessionHistory() } catch (e) { log('apply: history repair THREW ' + String(e)) }
  // 运行时注册自定义事件类型到 harness 的 KNOWN 集合：
  // 按绝对路径动态 import dsh 进程加载的同一个 dsh-session 模块（Node ESM 缓存按
  // 真实路径命中单例），给 KNOWN_SESSION_EVENT_TYPES 补上 status——持久化层读历史
  // 时即认识该类型，运行期新 append 的 status 事件不会导致历史不可读。
  // 失败不致命（repairSessionHistory 兜底），仅记录日志。
  try {
    const sessionLib = path.join(path.dirname(process.argv[1] || ''), '..', 'node_modules', '@deepseek-ai', 'dsh-session', 'lib', 'index.js')
    if (fs.existsSync(sessionLib)) {
      import(pathToFileURL(sessionLib).href)
        .then((m) => {
          if (m && m.KNOWN_SESSION_EVENT_TYPES) {
            if (!m.KNOWN_SESSION_EVENT_TYPES.has('dsh-graceful-restart/status')) {
              m.KNOWN_SESSION_EVENT_TYPES.add('dsh-graceful-restart/status')
              log('apply: status type registered into harness KNOWN set (' + sessionLib + ')')
            } else {
              log('apply: status type already in harness KNOWN set')
            }
          } else {
            log('apply: dsh-session module loaded but KNOWN set not found')
          }
        })
        .catch((e) => log('apply: status type register FAILED: ' + String(e)))
    } else {
      log('apply: dsh-session lib not found at ' + sessionLib + ' (register skipped)')
    }
  } catch (e) { log('apply: status type register THREW ' + String(e)) }
  ...
  // 统一推送"重启完成"：本进程是被重启执行器拉起的新代（重启已完成，服务就绪）。
  // 目标 = 触发重启的那个会话（marker 里记录的发起者，与唤醒目标一致）；
  // 会话恢复是异步的（客户端重连后 resume），轮询等待它成为 live 再发（无超时判断）。
  // 无发起会话（非唤醒重启）→ 不推送。
  if (restartedByExecutor) {
    const target = marker.sessionIds.length > 0 ? marker.sessionIds[0] : null
    // 注意：clearInterval 对 Cordis 的 interval 句柄不可靠（实测无效），
    // 用 pushed 标志位保证只推送一次；interval 仅作轮询等待。
    let pushed = false
    const iv = ctx.setInterval(() => {
      if (pushed) { clearInterval(iv); return }
      if (refreshReported && !refreshLocalOk) {
        // 刷新自检失败：新一代将被判为故障代，不再推送
        clearInterval(iv)
        return
      }
      if (!refreshLocalOk) return
      const s = target ? ctx.sessions.list().find((x) => x.id === target) : null
      if (target && !s) return // 目标会话尚未恢复，继续等
      pushed = true
      clearInterval(iv)
      try {
        pushStatus(ctx, 'restarted', 'DSH 重启完成，服务已恢复', target)
        log('pushStatus restarted')
      } catch (e) { log('pushStatus restarted THREW ' + String(e)) }
    }, 1000)
    ctx.effect(() => () => clearInterval(iv))
  }
