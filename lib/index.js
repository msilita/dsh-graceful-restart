...
/** 本进程由 dsh-graceful-restart launcher 拉起（IPC 通知启动器即可，无需 executor）。 */
const LAUNCHER_MODE = process.env.DSH_LAUNCHED_BY === 'dsh-graceful-restart-launcher'
...
function checkIdleAndExit(ctx) {
  ...
  if (LAUNCHER_MODE) {
    // 启动器模式：通过 IPC 通知常驻启动器（杀旧 + 同控制台拉起新实例），然后本进程退出
    const message = pendingAction === 'shutdown' ? 'shutdown' : 'restart'
    try {
      if (typeof process.send === 'function') process.send({ type: message })
      log('launcher notified: ' + message)
...
/* ------------------------------------------------------------------ */
/* 容器化重启（进程内重装）：架空 loader 的装载，实现"卸载+清缓存+重建" */
/* ------------------------------------------------------------------ */

let reloadArmed = false

/** 清掉"非 @deepseek-ai"的已加载插件模块缓存，让重装时 import 新代码。 */
function clearPluginCaches(loader) {
  try {
    const internal = loader && loader.internal
    if (internal && internal.loadCache) {
      for (const url of [...internal.loadCache.keys()]) {
        if (url.includes('node_modules') && !url.includes('@deepseek-ai/')) {
          Map.prototype.delete.call(internal.loadCache, url)
        }
      }
    }
  } catch (e) { log('reload: loadCache clear THREW ' + String(e)) }
  try {
    const { createRequire } = require('node:module')
    const req = createRequire(import.meta.url)
    for (const filepath of Object.keys(req.cache)) {
      if (filepath.includes('node_modules') && !filepath.includes('@deepseek-ai')) {
  ...
  } catch (e) { log('reload: require.cache clear THREW ' + String(e)) }
}

/**
 * 进程内重装：卸载 include 树中除本插件外的全部 entry（副作用回滚，
 * 持久化会话保留），清插件模块缓存，然后按原配置重建（loader 按服务依赖
 * 自动调度装载顺序）。进程不退出 → 控制台/前端会话不变。
 */
async function reloadInPlace(ctx) {
  const loader = ctx.get('loader')
  if (!loader) { log('reload: no loader service'); return { ok: false, error: 'no loader' } }
  const entries = []
  for (const entry of loader.entries()) {
    const options = entry && entry.options
    if (!options || options.id === 'dsh-graceful-restart') continue
    // parent 参数要求 group 的 entry id（null = root group）：
    // 找到持有 entry.parent 的 group entry（entry.subgroup === parent）
    let parentId = null
    for (const candidate of loader.entries()) {
      if (candidate.subgroup && candidate.subgroup === entry.parent) { parentId = candidate.options.id; break }
    }
    entries.push({ options, parentId, tree: entry.parent && entry.parent.tree })
  }
  log('reload: unloading ' + entries.length + ' entries')
  // 1. 卸载（isDispose=true：保留 options 数据供重建）
  for (const item of entries) {
    try {
      const entry = resolveEntryByOptions(loader, item.options)
      if (entry) await entry.parent.remove(item.options.id, true)
    } catch (e) { log('reload: unload ' + item.options.id + ' THREW ' + String(e)) }
  }
  // 2. 清插件模块缓存（非 @deepseek-ai，避免核心包双实例）
  clearPluginCaches(loader)
  // 3. 重建（串行；group 结构按 parentId 恢复；loader 的服务依赖自动调度）
  log('reload: rebuilding')
  const failures = []
  for (const item of entries) {
    try {
      await item.tree.create(item.options, item.parentId, Infinity)
    } catch (e) {
      failures.push(item.options.id + ': ' + String((e && e.message) || e))
      log('reload: rebuild ' + item.options.id + ' THREW ' + String((e && e.message) || e))
    }
  }
  log('reload: rebuilt ok=' + (entries.length - failures.length) + ' failed=' + failures.length)
  return { ok: failures.length === 0, failures }
}

/** 在 loader 树中按 options.id 找到当前存活的 entry。 */
function resolveEntryByOptions(loader, options) {
  if (!options || typeof options.id !== 'string') return null
  try {
    for (const entry of loader.entries()) {
      if (entry.options && entry.options.id === options.id) return entry
    }
  } catch {}
  return null
}

/** 等所有 agent idle 后执行进程内重装（工具触发后由本调度器执行）。 */
function scheduleReloadInPlace(ctx) {
  ...
  const iv = ctx.setInterval(() => {
    ...
    clearInterval(iv)
    reloadArmed = false
    log('reload: all agents idle, executing in-place reload')
    reloadInPlace(ctx).then((r) => {
      log('reload: done ok=' + r.ok + (r.failures && r.failures.length ? ' failures=' + JSON.stringify(r.failures) : ''))
      if (!r.ok) log('reload: FAILED — use shutdown/restart to recover')
    }).catch((e) => log('reload: THREW ' + String(e)))
  }, 1000)
  ctx.effect(() => () => clearInterval(iv))
}
...
export function apply(ctx) {
  ...
  if (wake) {
    ...
    try { tryResumeSessions(ctx, sessionIds, dynamic().continuePrompt) } catch (e) { log('apply: resume THREW ' + String(e)) }
  ...
  try {
    ...
    ctx.commands.register({
      name: 'reload',
      description: '进程内重启：等当前轮次结束后卸载并重装全部插件（不退出进程，新代码生效）',
      recordInput: false,
      handler() {
        scheduleReloadInPlace(ctx)
        return { kind: 'success', text: '进程内重启已安排：等当前轮次结束后卸载并重装全部插件。' }
      },
    })
  ...
  if (tools !== undefined) {
    try {
      ...
      ctx.effect(() => tools.register({
        name: 'restart_in_place',
        description:
          '进程内重启（容器化）：不退出进程，卸载全部插件并重新装载（清模块缓存，新代码/新插件生效）。'
          + '等当前轮次结束后执行；执行期间 web 端口短暂断开，自动重连；会话从持久化恢复。'
          + '返回安排结果。',
        parameters: {
          type: 'object',
          properties: {},
        ...
        async execute() {
          ...
          return {
            ok: true,
            message: '进程内重启已安排：等当前轮次结束后卸载并重装全部插件（进程不退出，控制台不变）。',
          }
        },
      }), 'dsh-graceful-restart: restart_in_place tool')
      log('apply: restart_in_place tool registered')
    } catch (e) {
