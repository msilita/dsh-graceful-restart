...
export async function apply(ctx) {
  ...
  // current 的 deps 与当前 loader 树，缺失的插件（包已安装但条目未加载，
  // 如拆根/配置异常导致的部分缺失）按快照动态装回（ctx.loader.create）。
  // 幂等：已加载的跳过；create 失败仅记录，不影响其他。
  setTimeout(() => {
    try {
      ...
      for (const e of loaderSvc.entries()) {
        if (e && e.options && typeof e.options.name === 'string') loaded.add(e.options.name)
      }
      for (const [pkg, spec] of Object.entries(deps)) {
        if (loaded.has(pkg)) continue
        const id = pkg.replace(/^@/, '').replace(/[^a-z0-9-]/gi, '-').toLowerCase()
        log('resume: snapshot plugin ' + pkg + ' not loaded, dynamically re-adding (' + String(spec) + ')')
        try {
          loaderSvc.create({ id, name: pkg })
            .then(() => log('resume: dynamically loaded plugin ' + pkg))
