...
async function wrapperApply(ctx) {
  ...
  // 拆树（第一代变空壳）：由 refresh-ok IPC 触发（第二代完全就绪的确定性信号，
  // 此时第一代自身 boot 校验必然已完成）。启动阶段拆树会被 assertEntriesActivated
  // 判 did-not-activate 导致第一代崩溃（实测），故只能在校验后拆。
  ...
  const spawnChild = (wake, rollbackRetry = false) => {
    ...
    child.on('message', (msg) => {
      ...
      if (msg.type === 'restart') {
        ...
        refreshOk = true
        markSuccess()
        // 第二代完全就绪 → 此时第一代自身的 boot 校验必然已完成
        // （第一代先启动，校验在 apply 后即结束；refresh-ok 是第二代就绪的
        //  确定性信号，远晚于第一代校验）→ 安全拆树，第一代变空壳。
        // 启动阶段拆树会被 assertEntriesActivated 判 did-not-activate（实测崩溃），
        // 故必须以该 IPC 为触发而非固定延迟/提前拆。
        pruneTree()
