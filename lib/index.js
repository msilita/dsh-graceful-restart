...
function scheduleExit(ctx, action, ownerId) {
  ...
  console.log('[dsh-graceful-restart] ' + actionLabel + '已安排：等当前轮次结束后'
    ...
    + '等待期间终端将显示进度。')
  // 广播"即将重启/关闭"（客户端如 QQ 订阅并通知用户）
  try {
    if (action === 'shutdown') ctx.emit('dsh-graceful-restart/shutting-down')
    else ctx.emit('dsh-graceful-restart/restarting')
  } catch (e) { log('broadcast restarting THREW ' + String(e)) }
...
export function apply(ctx) {
  ...
  // 广播"重启完成"：本进程是被重启执行器拉起的新代（重启已完成，服务就绪）。
  // 延迟到连接就绪自检通过后发（此时服务真正可用），客户端（如 QQ）订阅后通知用户。
  if (restartedByExecutor) {
    const iv = ctx.setInterval(() => {
      if (refreshLocalOk || refreshReported) {
        clearInterval(iv)
        try {
          ctx.emit('dsh-graceful-restart/restarted')
          log('broadcast restarted')
        } catch (e) { log('broadcast restarted THREW ' + String(e)) }
      }
    }, 1000)
    ctx.effect(() => () => clearInterval(iv))
  }
