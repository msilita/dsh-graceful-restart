...
/** 读唤醒 marker（重启前记录的活跃会话 id 列表 + 可选继续提示）。 */
function readResumeMarker() {
  try {
    ...
    return { sessionIds, prompt }
  } catch { return { sessionIds: [], prompt: undefined } }
}
...
/**
 * 本进程启动时间（代际 ID）：由 process.uptime() 反推（当前时刻 − 已运行时长），
 * 是真实的进程启动时刻，毫秒级唯一，比 pid 可靠（pid 可能被系统复用）。
 * 客户端用它做"重启检测"的持久化对比。
 */
const STARTED_AT = Date.now() - Math.floor(process.uptime() * 1000)
...
function wrapperApply(ctx) {
  ...
  const spawnChild = (wake) => {
    ...
    console.log('[dsh-graceful-restart] 正式 dsh 已启动 pid=' + child.pid + '（下方输出来自正式实例）')
  }
...
export function apply(ctx) {
  ...
  if (wake) {
    ...
    try { tryResumeSessions(ctx, marker.sessionIds, prompt) } catch (e) { log('apply: resume THREW ' + String(e)) }
  ...
  // Web 端点：status（启动信号 + 代际 ID）+ ack（页面确认）
