  ...
  try {
    ...
    if (loaderSvc && typeof loaderSvc.entries === 'function' && typeof loaderSvc.remove === 'function') {
      ...
      for (const e of loaderSvc.entries()) {
        if (!e || !e.options || e.disabled) continue
        if (e.id === name) { keep.push(e.id); continue }
        const mod = typeof e.options.name === 'string' ? e.options.name : ''
        if (mod.startsWith('@deepseek-ai/')) { keep.push(e.id); continue }
        drop.push(e.id)
      }
      ...
      setTimeout(() => {
        ;(async () => {
          ...
          log('wrapper: entry list pruned, kept [' + keep.join(', ') + '], dropped ' + drop.length)
        })()
      }, 1000)
    }
