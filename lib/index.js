  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: (req, res) => {
      ...
      try {
        ...
        if (!cm) { res.end(JSON.stringify({ ok: false, error: 'clientModules service unavailable' })); return }
        const table = {}
        for (const [k, v] of (cm.table || new Map()).entries()) {
          table[k] = { clientPath: v && v.clientPath, rev: v && v.entry && v.entry.rev }
        }
        res.end(JSON.stringify({ ok: true, baseUrl: ctx.baseUrl || null, tableKeys: Object.keys(table), table }, null, 2))
      } catch (e) {
