...
async function wrapperApply(ctx) {
  ...
  //   信号 = 轮询所有条目 fiber 脱离 pending（= 全部激活，tree.await() 已 resolve），
  //   再让一拍 setImmediate：即使校验尚未执行，tree.await() resolve 的微任务
  //   必然先于 setImmediate 宏任务 → 校验同步先跑，拆根必然在其后。
  //   轮询间隔 50ms，检测的是第一代自身加载进度，无时间猜测。
  try {
      ...
      ;(async () => {
        try {
          for (;;) {
            ...
            const pending = all.filter((e) => e && e.fiber && e.fiber.state === 'pending')
