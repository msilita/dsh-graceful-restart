...
const FILE = {
  ...
  stderr: () => path.join(HOME(), 'dsh-graceful-restart-stderr.log'),
...
function readSnapshot() {
  try {
    ...
    for (let i = entries.length - 1; i >= 0; i--) {
      ...
      if (typeof h.deps === 'object') {
        ...
        ordered.push({ deps: h.deps, at: h.at, note: h.note, error: h.error, diff: h.diff })
      } else {
  ...
  } catch { return { history: [], current: null, rollbacks: 0, rollbackLog: null, wakeLog: null } }
}

function writeSnapshot(s) {
  try {
    // 序列化：history[0] 存完整 deps（最新基准）+ 相对更早版本的差集（diff 记录时固定）；
    // 更早基线只存"相邻差集"——diff = 该版本相对更早版本（history[i+1]）带来的变化（+ 方向）；
    ...
    const history = Array.isArray(s.history) && s.history.length > 0
      ? s.history.map((h, i) => {
        ...
        if (i === 0) {
          ...
          return { deps: h.deps, at: h.at, note: h.note, error: h.error, ...(diff !== undefined ? { diff } : {}) }
        }
...
/** 从 stderr log 文件读取最近的报错尾部（具体错误信息，供 error 记录/唤醒病历）。 */
function readStderrTail(maxChars) {
  try {
    const s = fs.readFileSync(FILE.stderr(), 'utf8')
    return s.trim().slice(-(maxChars || 2048))
  } catch { return '' }
}

/** 清空 stderr log（新代际/启动成功后）。 */
function clearStderrLog() {
  try { fs.writeFileSync(FILE.stderr(), '', 'utf8') } catch { /* best-effort */ }
}
...
function wrapperApply(ctx) {
  ...
  // 第二代存活超过宽限期 → 启动成功。
  // 历史无限保留，失败尝试（错误版本）保留在链上（带 error，视图 ⚠ + 下方完整显示）；
  // 当前与"任意完整基线"相同（回滚后退恢复）→ 不新增条目（指针后退语义）；
  // 否则新基线入列。清零回滚计数。
  const markSuccess = () => {
    successTimer = null
    ...
    const note = rollbackLog && rollbackLog.entries.length > 0
      ...
      : undefined
    const sameIdx = history.findIndex((h) => h.deps && depsEqual(h.deps, current))
    if (sameIdx >= 0) {
      // 当前与既有基线相同（回滚后退恢复）→ 不新增条目；note 记到该基线
      if (note && history[sameIdx].note === undefined) history[sameIdx].note = note
    } else {
  ...
  //   added   基线里没有的包 → 卸载
  //   changed 基线里有但版本/地址不同 → 卸载后重装基线版本（精确回到上一个版本）
  ...
  const spawnChild = (wake) => {
    ...
    // 尝试版本入列（失败版本带 error ⚠ 保留在链上；回滚=指针后退，恢复成功不新增）。
    // 入列条件：当前清单与 HEAD（history[0]）不同——即"新版本"；
    // 回滚重试中（rollbackHead 非 null）current 已回退到基线 → 不入列。
    const latest = readProfileDeps()
    if (latest !== null) {
      current = latest
      const headSame = history.length > 0 && history[0].deps && depsEqual(history[0].deps, current)
      const atBaseline = rollbackHead !== null && history[rollbackHead]
        && depsEqual(history[rollbackHead].deps, current)
      if (!headSame && !atBaseline) {
        // 新版本产生 → 若之前回滚过，丢弃比当前新的失败记录（回滚期间又装了新包）
    ...
    child.stderr.on('data', (c) => {
      try { fs.appendFileSync(FILE.stderr(), c) } catch { /* best-effort */ }
    ...
    child.once('exit', (code, sig) => {
      ...
      if (failed) {
        ...
        if (baseline) {
          ...
          // 1) 错误记到 history[0]（两级：detail 概要 + stderr 具体报错）
          const failDetail = '第二代启动失败（退出码 ' + code + '，' + Math.round(elapsed / 1000) + 's）'
          const failStderr = readStderrTail(2048)
          if (history.length > 0) history[0].error = { at: new Date().toISOString(), detail: failDetail, stderr: failStderr || undefined }
          ...
          restoreMarkerForRetry(failDetail + '，自动回滚卸载：' + (desc || '(无)') + '（基线版本 #' + baselineIdx + '）', failStderr)
