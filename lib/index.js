...
async function wrapperApply(ctx) {
  ...
  // 拆树（延迟执行，第一代变空壳）：**绝不能早于 dsh boot 校验**——
  // assertEntriesActivated 会把"被拆条目(fiber 缺失)"判为 did-not-activate，
  // 启动阶段拆树必然导致第一代崩溃（实测）。spawn 后延迟 3s（boot 校验通常
  // 在 apply 完成后 1s 内结束）再拆，拆完残留 pending 无碍（校验已过）。
  // 拆树链为原生 async（fiber dispose 不影响链）；只拆 include 的子条目
  // （include 根是配置树写回载体，拆它会写坏磁盘配置）。
  // 端口已由 spawn 前的 server.close 释放，拆树时 webServer 的 effect 再次关闭无妨。
  try {
  ...
  } catch (e) { log('wrapper: prune THREW ' + String(e)) }
}
