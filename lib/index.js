...
function checkIdleAndExit(ctx) {
  ...
  if (running.length > 0) {
    ...
    const now = Date.now()
    if (waitStartedAt === 0) waitStartedAt = now
    const label = pendingAction === 'shutdown' ? '关闭' : pendingAction === 'restart-wake' ? '重启+唤醒' : '重启'
    // 关闭的强制等待上限（45s）：用户显式关闭的意图优先于未完成的轮次——
    // 幽灵 agent（恢复后一直 running 的空会话）不应无限阻塞关闭。
    // 重启保持原语义（等轮次结束，70s 兜底重置），避免打断正在收尾的工作。
    if (pendingAction === 'shutdown' && now - waitStartedAt >= 45000) {
      log('shutdown wait exceeded 45s, forcing exit despite running agents')
      console.log('[dsh-graceful-restart] 关闭等待超时（45s）：以下会话仍未结束，'
        + '按用户关闭意图强制执行：' + running.map((a) => String(a.id).slice(0, 8)).join(', '))
      running.length = 0 // 视为空闲，继续走退出流程
    } else if (now - lastWaitPrint >= 5000) {
      lastWaitPrint = now
      console.log('[dsh-graceful-restart] ' + label + '等待中：还有 ' + running.length + ' 个会话在运行（'
        + running.map((a) => String(a.id).slice(0, 8)).join(', ') + '），轮次结束后自动执行...')
    }
