...
async function wrapperApply(ctx) {
  ...
  // 先拆根、再 spawn（第一代加载完成 → 卸载树 → 启动第二代）：
  //   顺序：close 端口（提前释放，拆根前即零端口）
  //         → await loader.await()（EntryTree.await：等第一代全部条目激活，
  //           与 boot 的 await 同条件；resolve 后微任务序保证 boot 校验先同步跑完）
  //         → setImmediate 一拍（校验确定完成）
  //         → remove('include', true) 拆根（第一代空壳：零插件零端口）
  //         → spawnChild(false) 启动第二代（spawn 时第一代已空壳，与第二代
  //           端口/插件完全无关，无双跑窗口）
  //   拆根链为后台 async（不 await，避免 apply 等自身 inertia 死锁；
  //   wrapperApply 返回后自身 apply 完成，loader.await() 才能 resolve）。
  //   约束（实测）：拆根不能早于第一代 boot 校验（assertEntriesActivated 判
  //   did-not-activate 崩溃）；不能用 fiber.state 轮询（启动早期未创建 fiber 漏判）。
  //   零插件名、与任何插件零关系。
  try {
    const loaderSvc = ctx.get('loader')
    const useAwait = !!(loaderSvc && typeof loaderSvc.await === 'function')
    log('wrapper: prune setup loader=' + (loaderSvc ? 'yes' : 'no') + ' loader.await=' + (useAwait ? 'yes' : 'NO'))
      ...
      ;(async () => {
        try {
          if (useAwait) {
            ...
            log('wrapper: loader.await settled, boot validation passed, pruning root')
          } else {
            // 兜底：拿不到 loader.await 时用 1.5s 延迟（15:10 轮实证的成功组合）
          ...
          try {
            ...
            log('wrapper: root prune THREW ' + String(e) + ' (degraded: spawn without prune)')
          }
          log('wrapper: spawning second generation')
          spawnChild(false)
          log('wrapper: complete (staying resident)')
        } catch (e) { log('wrapper: prune-then-spawn THREW ' + String(e)) }
      })()
    }
  } catch (e) { log('wrapper: prune-then-spawn THREW ' + String(e)) }
}
