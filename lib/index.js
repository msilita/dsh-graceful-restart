...
export function apply(ctx) {
  ...
  // 重启端点：供外部客户端（如 QQ 的 /restart 命令）触发"重启+唤醒"。
  // POST /plugins/dsh-graceful-restart/restart → scheduleExit('restart-wake')，
  // 返回已安排；调用方（qq-client）应先在发送端发"正在重启"通知。
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/plugins/dsh-graceful-restart/restart',
    handler: (req, res) => {
      if (req.method !== 'POST') {
        res.writeHead(405, { allow: 'POST' })
        res.end('method not allowed')
      ...
      req.on('end', () => {
        ...
        try {
          ...
          if (typeof p.continuePrompt === 'string' && p.continuePrompt) prompt = p.continuePrompt
        } catch { /* 无 body 或非 JSON，用默认 */ }
        if (prompt) pendingContinuePrompt = prompt
        scheduleExit(ctx, 'restart-wake', null) // owner=null：HTTP 触发，无发起会话
        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' })
        res.end(JSON.stringify({ ok: true, message: '重启已安排' }))
      })
    },
  }), 'dsh-graceful-restart: restart endpoint')
