...
/**
 * client bundle 静态检查：DSH 的 client bundle 经 `__ModuleLoader__.load({ factory })`
 * 包装，factory 内模块导出的**合法写法**有两种（均支持）：
 *   A. CommonJS 风格：`var module = {exports:{}}; var exports = module.exports; exports.x=...; return module.exports`
 *   B. 直接返回对象：`return { inject, apply }`（不经过 exports/module.exports）
 * 检查原则：**只对真正使用 `exports.` / `module.exports` 的 bundle 要求对应定义**
 * （"使用即检查"）——B 风格不误判；缺失定义会在浏览器执行时抛
 * `exports is not defined`（页面显示 "Failed to load plugins"，但 host 启动不执行
 * client.js，boot 输出扫描与 HTTP 探测都抓不到——这是唯一能提前识别的层）。
 * 返回 null = 就绪；否则返回失败原因。 */
function checkClientBundleSource(src) {
  if (!src.includes('__ModuleLoader__.load')) return '缺少 __ModuleLoader__.load 模块包装'
  const usesExports = /(^|[^.\w])exports\./.test(src)
  const definesExports = /(var|const|let)\s+exports\s*=/.test(src)
  if (usesExports && !definesExports) return 'client.js 使用了 exports 但未定义（执行时会抛 exports is not defined）'
  const usesModuleExports = /\bmodule\.exports/.test(src)
  const definesModule = /(var|const|let)\s+module\s*=\s*\{\s*exports\s*:/.test(src)
  if (usesModuleExports && !definesModule) return 'client.js 使用了 module.exports 但未定义 module'
  return null
}

/** 探测一个插件的 client bundle（页面级启动失败检测）：
 *  两层检查：HTTP 可达性 + 精确静态检查（exports/module 定义）。
 *  另由 boot 输出扫描（detectBootFailure）兜底其他类型加载错误。
 *  返回 null = 就绪；否则返回失败原因。 */
async function probeClientBundle(pkg) {
  ...
  const check = checkClientBundleSource(src)
