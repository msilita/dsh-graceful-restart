...
/** 追踪会话活跃：只认**真实对话事件**——空会话（无任何用户消息/轮次）不算活跃。
 *  曾出现：11:28 创建的空会话（无 user/message）被 marker 记录并唤醒，
 *  失控跑工具循环阻塞重启。现在只有 user/message、turn/start、turn/end
 *  才刷新活跃时间；agent/status 的 running 也算（真实工作中的会话）。 */
function trackSessionActivity(ctx) {
  try {
    ...
    // session/event：只认真实对话事件（用户消息/轮次边界）——空会话的事件
    // （session/end-seed、chunk 等）不算活跃，避免空会话被唤醒
