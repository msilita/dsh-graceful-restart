...
function wrapperApply(ctx) {
  ...
  // 先拆树、再 spawn：拆树链为原生 async（不依赖 Cordis fiber 存活——实测
  // fiber dispose 后 async 链继续执行），链尾调用 spawnChild（原生闭包）。
  // 拆树顺序：webServer 优先（其 fiber effect 清理会关闭 HTTP 监听 → 端口释放），
  // 其余随后；全部拆完（第一代零插件、零端口）才 spawn 第二代，
  // 与第二代端口配置完全无冲突（无需单独 close——拆树 effect 自然关闭端口）。
