...
function wrapperApply(ctx) {
  ...
  // 机制：ctx.loader.update(config) 整体替换条目列表 → 对不在新列表的条目逐个
  // remove(id, true)（dispose-only，不改配置文件，重启后第二代配置完好）。
  // 从结构上保证第一代零业务插件（无双 bot 连接 / 双定时器等双跑副作用），
  // 且新装插件无需任何配置配合（不在白名单 → 自动被 diff 卸载）。
  ...
  try {
    ...
    if (loaderSvc && typeof loaderSvc.entries === 'function' && typeof loaderSvc.update === 'function') {
      const entries = [...loaderSvc.entries()]
      const next = entries
        .filter((e) => {
          if (!e || !e.options) return false
          if (e.id === name) return true // 自身
          const mod = typeof e.options.name === 'string' ? e.options.name : ''
          return mod.startsWith('@deepseek-ai/')
        })
        .map((e) => e.options)
      // 延迟执行：等 spawn 流程稳定后再替换（异步，失败不影响 wrapper 使命）
      setTimeout(() => {
        loaderSvc.update(next)
          .then(() => log('wrapper: entry list replaced, kept ' + next.length + ' entries'))
          .catch((e) => log('wrapper: loader.update THREW ' + String(e)))
      }, 1000)
    }
