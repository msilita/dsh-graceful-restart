...
function wrapperApply(ctx) {
  log('wrapper: first generation, pruning tree then spawning real dsh')
  ...
  // 先拆树、再 spawn（末尾执行：spawnChild 等 const 声明均已初始化，避免 TDZ）：
  // 拆树链为原生 async（不依赖 Cordis fiber 存活——实测 fiber dispose 后 async
  // 链继续执行），链尾调用 spawnChild（闭包）。拆树顺序：webServer 优先
  // （其 fiber effect 清理会关闭 HTTP 监听 → 端口释放），其余随后；
  // 全部拆完（第一代零插件、零端口）才 spawn 第二代，与第二代端口配置完全无冲突
  // （无需单独 close——拆树 effect 自然关闭端口）。
  // ★ include 根绝不能拆：include 是配置树的写回载体，拆它会把内存中被拆后的
  //   配置树写回磁盘，破坏 profile 配置（实测导致插件从 bundles 消失）。
  //   只拆 include 的子条目即可（效果同样：第一代空壳）。
  const loaderSvc = ctx.get('loader')
  if (loaderSvc && typeof loaderSvc.entries === 'function' && typeof loaderSvc.remove === 'function') {
    ;(async () => {
      const entries = [...loaderSvc.entries()].filter((e) => e && e.id !== undefined && String(e.id) !== 'include')
      // webServer 优先拆（先释放端口）
      entries.sort((a, b) => {
        const an = a && a.options && typeof a.options.name === 'string' ? a.options.name : ''
        const bn = b && b.options && typeof b.options.name === 'string' ? b.options.name : ''
        return (an === '@deepseek-ai/dsh-host-webserver' ? -1 : 0) - (bn === '@deepseek-ai/dsh-host-webserver' ? -1 : 0)
      })
      ...
      log('wrapper: tree pruned (bare process), spawning second generation')
      spawnChild(false)
      log('wrapper: complete (staying resident)')
    })()
    console.log('[dsh-graceful-restart] 第一代 wrapper：正在卸载插件树并启动正式 dsh（几秒内完成）...')
    return
  }
...
export function apply(ctx) {
  ...
  // 第一代（用户直接启动）：先拆树（零插件、零端口）→ spawn 第二代 → 常驻 launcher
