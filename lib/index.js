...
async function wrapperApply(ctx) {
  ...
  // 先释放端口再 spawn：关闭第一代 webServer 的 HTTP 监听（server.close 直接关闭
  // 底层 node:http server——WebServer 无公开 stop，server 为普通实例字段），
  // 保证 spawn 第二代时第一代零端口占用，从根上杜绝端口冲突（与第二代端口配置无关）。
  ...
  // 极致清场（设计目标）：第一代拆掉整棵插件树——官方 core 与自身全部卸载，
  // 只剩"空壳进程"。进程级 wrapper 功能（spawn 第二代、IPC restart/shutdown、
  // guard 定时器、exit 处理）全部注册在原生事件/闭包上，不依赖 Cordis fiber 存活，
  // fiber dispose 不影响它们（实测：拆根后进程存活、第二代正常运行）。
  // 端口：patch 已设 port 0（OS 随机），拆根后 webServer 也卸载 → 第一代零固定端口、零 HTTP。
  // 注意：第一代本身不提供任何服务；守护逻辑在第二代内部（插件），
  // 第二代意外退出时第一代跟随退出（看门狗式自动拉起由外部承担，属设计取舍）。
  try {
    ...
    if (loaderSvc && typeof loaderSvc.entries === 'function' && typeof loaderSvc.remove === 'function') {
      ...
      setTimeout(() => {
        ;(async () => {
          ...
          log('wrapper: full prune complete (first generation is a bare process)')
        })()
      }, 1000)
    }
  } catch (e) { log('wrapper: full prune THREW ' + String(e)) }
}
...
export async function apply(ctx) {
  ...
  // 第一代（用户直接启动）：阻止启动 → 释放端口 → spawn 第二代 → 常驻 launcher
