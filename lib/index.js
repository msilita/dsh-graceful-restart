...
async function wrapperApply(ctx) {
  ...
  // 先拆根、再启动第二代：1s 延迟保证第一代 boot 校验完成（15:10 实证的时间点）
  // → remove('include', true) 拆根（第一代空壳：零插件零端口）
  // → 拆根完成后才 spawnChild 启动第二代（spawn 时第一代已空壳，
  //   与第二代端口/插件完全无关，无双跑窗口）。
  // 拆根失败 = 整体失败：不启动第二代（避免半拆状态 spawn 的未定义行为）。
  // 零插件名、与任何插件零关系。
  // 对照实验开关：DSH_GR_DISABLE_PRUNE=1 时禁用拆根、直接 spawn（排查拆根
  // 是否影响第二代 qq/waker 等插件的加载）。
  if (process.env.DSH_GR_DISABLE_PRUNE === '1') {
    log('wrapper: prune DISABLED (DSH_GR_DISABLE_PRUNE=1), spawning directly')
    spawnChild(false)
  ...
  try {
    ...
    if (loaderSvc && typeof loaderSvc.remove === 'function') {
      setTimeout(() => {
        loaderSvc.remove('include', true)
          .then(() => {
            log('wrapper: root pruned (first generation is a bare process)')
            log('wrapper: spawning second generation')
            spawnChild(false)
            log('wrapper: complete (staying resident)')
          })
          .catch((e) => log('wrapper: root prune THREW ' + String(e) + ' (NOT spawning second generation)'))
      }, 1000)
    }
...
export async function apply(ctx) {
  ...
  // 快照驱动的插件自愈：启动完成后（3s，等全部条目稳定），对比 guard 快照
  // current 的 deps 与当前 loader 树：
  //   · 条目缺失（包已安装但未加载）→ ctx.loader.create 动态装回
  //   · 条目存在但 fiber 未激活（pending/无 fiber，如 apply 失败/拆根残留）→
  //     先 remove(id, true) 再 create 重建（重新激活）
  // 幂等：已激活的跳过；失败仅记录。
  setTimeout(() => {
    try {
      const snap = readSnapshot()
      const cur = snap.current
      const target = snap.history.find((h) => h.at === cur) || snap.history[0]
      const deps = target && typeof target.deps === 'object' ? target.deps : {}
      const loaderSvc = ctx.get('loader')
      if (!loaderSvc || typeof loaderSvc.entries !== 'function' || typeof loaderSvc.create !== 'function') return
      const byName = new Map()
      for (const e of loaderSvc.entries()) {
        if (e && e.options && typeof e.options.name === 'string') {
          if (!byName.has(e.options.name)) byName.set(e.options.name, e)
        }
      }
      for (const [pkg, spec] of Object.entries(deps)) {
        const entry = byName.get(pkg)
        const active = !!(entry && entry.fiber && entry.fiber.state !== 'pending')
        log('resume: snapshot check ' + pkg + ' -> ' + (entry ? (active ? 'active' : 'INACTIVE (fiber=' + (entry.fiber ? entry.fiber.state : 'none') + ')') : 'MISSING'))
        if (active) continue
        const id = pkg.replace(/^@/, '').replace(/[^a-z0-9-]/gi, '-').toLowerCase()
        log('resume: re-adding plugin ' + pkg + ' (' + String(spec) + ')')
        try {
          ;(async () => {
            try {
              if (entry && loaderSvc.remove) await loaderSvc.remove(entry.id, true)
            } catch (e) { log('resume: remove ' + pkg + ' THREW ' + String(e && e.message || e)) }
            await loaderSvc.create({ id, name: pkg })
            log('resume: dynamically loaded plugin ' + pkg)
          })().catch((e) => log('resume: dynamic load ' + pkg + ' FAILED ' + String(e && e.message || e)))
        } catch (e) { log('resume: dynamic load ' + pkg + ' THREW ' + String(e)) }
      }
    } catch (e) { log('resume: snapshot self-heal THREW ' + String(e)) }
  }, 3000)
}
