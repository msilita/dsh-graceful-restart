 ...
 *     多版本（history，数量由设置 snapshotKeep 决定，默认 3）；
 ...
 *     第二代存活超过宽限期 → 新基线入列（与最新相同则不重复），截断到 snapshotKeep。
...
const ConfigSchema = z.object({
  ...
  // 启动守护：保留的成功基线快照版本数（1-10）。回滚时逐级使用：
  // 第 1 次回滚到最近成功基线，仍失败则回滚到更早的基线。
  // step(1) = 整数约束（schemastery 无 .int()）
  snapshotKeep: z.number().step(1).min(1).max(10).default(3),
})
...
/**
 * 读取并规范化启动守护快照。
 * 结构：{ history: [{deps, at}, ...]（成功基线，最新在前）, current: {deps, at}, rollbacks }
 * 兼容旧版单值结构：{ ok: {deps, at}, current, rollbacks } → history = [ok]。
 */
function readSnapshot() {
  try {
    const s = JSON.parse(fs.readFileSync(FILE.snapshot(), 'utf8'))
    if (!s || typeof s !== 'object') return { history: [], current: null, rollbacks: 0 }
    let history = Array.isArray(s.history)
      ? s.history.filter((h) => h && typeof h.deps === 'object')
      : []
    // 旧结构兼容：仅当没有 history 时才采用单值 ok
    if (history.length === 0 && s.ok && typeof s.ok.deps === 'object') history = [s.ok]
    return {
      history,
      current: s.current && typeof s.current.deps === 'object' ? s.current : null,
      rollbacks: typeof s.rollbacks === 'number' ? s.rollbacks : 0,
    }
  } catch { return { history: [], current: null, rollbacks: 0 } }
}
...
function writeSnapshot(s) {
  try {
    ...
    const payload = {
      ...s,
      ok: Array.isArray(s.history) && s.history.length > 0 ? s.history[0] : null,
    }
...
function wrapperApply(ctx) {
  ...
  // 读取 snapshotKeep：成功基线保留几个版本（设置菜单可调）
  ...
  const guardKeep = () => {
    const v = Number(resolveGuardConfig().snapshotKeep)
    return Number.isFinite(v) ? Math.min(10, Math.max(1, Math.floor(v))) : DEFAULT_CONFIG.snapshotKeep
  }
  ...
  log('guard: snapshotKeep=' + guardKeep())
  ...
  // 第二代存活超过宽限期 → 启动成功 → 新基线入列（与最新相同则不重复记录），
  // 截断到 snapshotKeep 个，并清零回滚计数
  const markSuccess = () => {
    successTimer = null
    ...
    if (!sameAsHead) {
      history = [{ deps: current, at: new Date().toISOString() }, ...history].slice(0, guardKeep())
    }
    rollbacks = 0
    persist()
  ...
  // 卸载"指定基线之后新增"的插件（官方 CLI：node bin.js plugin --profile <p> remove <pkg>）
  const rollbackNewPlugins = (candidate, baseline) => {
    const added = Object.keys(candidate).filter((k) => !(k in baseline))
    if (added.length === 0) return 0
    console.log('[dsh-graceful-restart] 启动失败，自动卸载该基线之后新安装的插件：' + added.join(', '))
    let removed = 0
    for (const pkg of added) {
      log('guard: removing ' + pkg)
      try {
        const r = spawnSync(process.execPath, [process.argv[1], 'plugin', '--profile', profile, 'remove', pkg], {
          cwd: profileDir(),
          stdio: ['ignore', 'inherit', 'inherit'],
          timeout: 120000,
        })
        if (r.status === 0) {
          removed++
          log('guard: removed ' + pkg)
        } else {
          log('guard: remove ' + pkg + ' status=' + r.status + (r.error ? ' err=' + String(r.error) : ''))
        }
      } catch (e) {
        log('guard: remove ' + pkg + ' THREW ' + String(e))
      }
    }
    return removed
  }
  ...
  // 探测新增插件的 client bundle（页面级启动失败检测）：
  ...
  const scheduleClientProbe = (childRef) => {
    ...
    // 临时：探测延迟 30s（排查 dsh-waker 404——给坏代唤醒留窗口）；排查后改回 5s
    setTimeout(async () => {
      ...
      for (const pkg of added) {
        ...
        if (!ok) {
          ...
          // 临时：排查 dsh-waker 404 期间不 kill（保留现场）；排查后恢复 kill
  ...
  const spawnChild = (wake) => {
    ...
    if (latest !== null) {
      current = latest
      persist()
      log('guard: refreshed current deps [' + Object.keys(current).join(', ') + ']')
    }
    ...
    child.once('exit', (code, sig) => {
      ...
      // 第 N 次回滚使用 history[N-1]（最新 → 更早逐级回溯）
      if (failed && history.length > 0 && rollbacks < MAX_ROLLBACKS) {
        const baseline = history[Math.min(rollbacks, history.length - 1)].deps
        const added = Object.keys(current).filter((k) => !(k in baseline))
        if (added.length > 0) {
          rollbacks++
          restoreMarkerForRetry('第二代启动失败（退出码 ' + code + '，' + Math.round(elapsed / 1000)
            + 's），自动回滚卸载新增插件：' + added.join(', '))
          console.log('[dsh-graceful-restart] 第二代启动失败（退出码 ' + code + '，' + Math.round(elapsed / 1000)
            + 's），第 ' + rollbacks + ' 次回滚重试（基线版本 #' + Math.min(rollbacks - 1, history.length - 1) + '）...')
          rollbackNewPlugins(current, baseline)
          // remove 后重新读取清单（以回滚后状态为新基线）并更新快照
          const after = readProfileDeps()
          current = after !== null ? after : baseline
          persist()
...
export function apply(ctx) {
  ...
  // 临时调试端点：client-modules 注册表状态（排查 dsh-waker client 404 根因）
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/plugins/dsh-graceful-restart/debug-cm',
    handler: (req, res) => {
      res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' })
      try {
        ...
        if (loader && typeof loader.entries === 'function') {
          for (const entry of loader.entries()) {
            entries.push({
              ...
              fiber: entry.fiber ? 'active' : 'none',
              resolved: entry.resolved,
            })
          }
        }
        const table = {}
        for (const [k, v] of (cm && cm.table ? cm.table : new Map()).entries()) {
          table[k] = { clientPath: v && v.clientPath, rev: v && v.entry && v.entry.rev }
        }
        res.end(JSON.stringify({ ok: true, baseUrl: ctx.baseUrl || null, tableKeys: Object.keys(table), table, loaderEntries: entries }, null, 2))
      } catch (e) {
        res.end(JSON.stringify({ ok: false, error: String(e) }, null, 2))
      }
    },
  }), 'dsh-graceful-restart: debug client-modules')
