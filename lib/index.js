...
function readSnapshot() {
  try {
    ...
    // 错误记录：独立 errors 数组；旧格式条目内 error → 迁移收集（不留在快照条目上）
...
function wrapperApply(ctx) {
  ...
  // 第二代存活超过宽限期 → 启动成功。正常启动后不再操作快照：
  // 快照/指针在 spawn 时已定（差集空 → 不动；非空 → 新快照 + current 指向）。
  // 这里只做收尾：清空 stderr log、若曾回滚则追加"恢复成功"记录。
  const markSuccess = () => {
    successTimer = null
    if (!child || child.exitCode !== null) return
    log('guard: second generation alive past grace, marking success')
  ...
  const rollbackPlugins = (candidate, baseline) => {
    ...
    for (const pkg of removed) {
      ...
      if (runCli(['add', String(spec)])) log('guard: re-added ' + pkg)
  ...
  const spawnChild = (wake) => {
    ...
    //   差集为空 → 不变化（current 不动）；
    //   差集非空 → 建立新快照（修改快照链），current 指针指向新快照（向更新移动）。
    ...
    child.once('exit', (code, sig) => {
      ...
      if (failed) {
        ...
        // 错误独立记录（两级：detail 概要 + stderr 具体报错）
        errors.push({ at: new Date().toISOString(), detail: failDetail, ...(failStderr ? { stderr: failStderr } : {}) })
