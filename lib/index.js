...
function wrapperApply(ctx) {
  ...
  // 白名单替换：第一代只保留官方 core 与自身，第三方业务插件全部从条目列表移除。
  // 机制：枚举 ctx.loader.entries()，对白名单外的条目逐个 remove(id, true)
  // （dispose-only，不改配置文件，重启后第二代配置完好）——结果等价于把条目列表
  // 整体替换为白名单（loader.update 内部 diff 后同样是逐个 remove，这里直接做，
  // 避免 update 对部分 options 结构的规范化问题）。
  // 从结构上保证第一代零业务插件（无双 bot 连接 / 双定时器等双跑副作用），
  // 且新装插件无需任何配置配合（不在白名单 → 自动被移除）。
  // 白名单策略：@deepseek-ai/* 官方 core（含自身 inject 依赖闭包）+ 自身；
  // 若未来要极致白名单（仅自身 + 树根），需先把静态 inject 缩到最小
  // （卸载 inject 服务会把本插件挂回 PENDING）。
  try {
    ...
    if (loaderSvc && typeof loaderSvc.entries === 'function' && typeof loaderSvc.remove === 'function') {
      ...
      for (const e of loaderSvc.entries()) {
        ...
        const opt = e.options && typeof e.options === 'object' ? e.options : {}
        const mod = typeof opt.name === 'string' ? opt.name : ''
        const id = String(e.id)
        // 树根/结构条目（无模块名，如根 include、随机 id 的组）：绝不能拆（拆根 = 拆整棵树）
        if (!mod || id === 'include') { keep.push(id); continue }
        // 官方 core + 自身（按模块名识别，条目 id 是 include:xxx 复合路径）
        if (mod.startsWith('@deepseek-ai/') || mod === name) { keep.push(id); continue }
        drop.push(id)
      }
      // 延迟执行：等 spawn 流程稳定后再清场（异步，失败不影响 wrapper 使命）
      setTimeout(() => {
        ;(async () => {
          for (const id of drop) {
            try { await loaderSvc.remove(id, true); log('wrapper: pruned entry ' + id) }
            catch (e) { log('wrapper: prune ' + id + ' THREW ' + String(e)) }
          }
          log('wrapper: entry list pruned, kept ' + keep.length + ', dropped ' + drop.length + ' [' + drop.join(', ') + ']')
        })()
      }, 1000)
    }
  } catch (e) { log('wrapper: whitelist replace THREW ' + String(e)) }
}
