...
function executorScript() {
  return String.raw`// dsh-graceful-restart executor: one-shot restart worker (node + koffi FFI).
// detached 启动（无控制台，父 DSH 退出后存活）；等旧 DSH 退出后
// AttachConsole 附着用户终端（旧 DSH 的父进程）并重绑 std 句柄，
// 再以 inherit 方式拉起新 DSH（输出回到用户终端）。
...
const userShellPid = Number(process.argv[3] || 0)
const wake = process.argv[4] === '1'
const koffiPath = process.argv[5] || ''
...
log('executor started pid=' + process.pid + ' oldPid=' + oldPid + ' userShell=' + userShellPid + ' wake=' + wake + ' koffi=' + (koffiPath ? 'yes' : 'no'))
...
function relaunch() {
  ...
  // 附着用户终端 + 重绑标准句柄（koffi FFI）
  let attachOk = false
  try {
    const koffi = require(koffiPath)
    const lib = koffi.load('kernel32.dll')
    const FreeConsole = lib.func('bool FreeConsole()')
    const AttachConsole = lib.func('bool AttachConsole(uint32)')
    const SetStdHandle = lib.func('bool SetStdHandle(int32,intptr)')
    const CreateFileA = lib.func('intptr CreateFileA(char*,uint32,uint32,void*,uint32,uint32,intptr)')
    FreeConsole()
    if (userShellPid > 0) attachOk = AttachConsole(userShellPid)
    if (!attachOk && oldPid > 0) attachOk = AttachConsole(oldPid)
    if (attachOk) {
      const hIn = CreateFileA('CONIN$', 2147483648, 3, null, 3, 0, 0n)
      const hOut = CreateFileA('CONOUT$', 3221225472, 3, null, 3, 0, 0n)
      SetStdHandle(-10, hIn)
      SetStdHandle(-11, hOut)
      SetStdHandle(-12, hOut)
    }
    log('attach console to userShell=' + userShellPid + ' result=' + attachOk)
  } catch (e) {
    log('attach console threw: ' + String(e))
  }

  const argv = [].concat(idx.execArgv || [], idx.argv || [])
  const child = spawn(idx.execPath, argv, {
    cwd: idx.cwd || process.cwd(),
    // inherit：继承 AttachConsole + SetStdHandle 修复后的控制台句柄
    stdio: attachOk ? 'inherit' : ['ignore', 'ignore', 'ignore'],
    env: { ...process.env, DSH_GRACEFUL_RESTART_WAKE: wake ? '1' : '0' },
  })
  child.once('error', (e) => { log('relaunch spawn error: ' + String(e)); process.exit(0) })
  log('relaunch spawned pid ' + child.pid + (attachOk ? ' (console attached)' : ' (no console)'))
...
/** Spawn the one-shot executor (detached node: survives parent exit; no IPC). Returns the child or null. */
function spawnExecutor(action) {
  try {
    fs.writeFileSync(FILE.executor(), executorScript(), 'utf8')
    const wake = action === 'restart-wake' ? '1' : '0'
    // 解析 koffi（FFI 库）的绝对路径传给执行器：DSH profile 的共享依赖层里有它
    let koffiPath = ''
    try { koffiPath = createRequire(import.meta.url).resolve('koffi') } catch { /* fallback: executor will skip attach */ }
    ...
    child.unref()
    log('executor spawned pid ' + child.pid + ' action=' + action + ' ppid=' + (process.ppid || 0) + ' koffi=' + (koffiPath ? 'yes' : 'no'))
    return child
  } catch (error) {
