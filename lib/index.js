 ...
 *  除 HTTP 200 外还做静态包装检查——浏览器端 client bundle 必须以
 *  `window.__ModuleLoader__.load({ factory: (require) => { var module = {exports:{}}; ... } })`
 *  包装。缺失 module/exports 定义会在执行时抛 `exports is not defined`
 *  （页面 "Failed to load plugins"，但 HTTP 仍 200，纯 HTTP 探测抓不到）。
 *  返回 null = 就绪；否则返回失败原因。 */
async function probeClientBundle(pkg) {
  ...
  if (!/\bmodule\s*=\s*\{\s*exports\s*:/.test(src) || !/\bexports\s*=\s*module\.exports/.test(src)) {
    return 'client.js 未定义 module/exports（执行时会抛 exports is not defined）'
  }
  return null // 就绪
}
...
function wrapperApply(ctx) {
  ...
  const spawnChild = (wake) => {
    ...
    // 第二代 stderr → pipe → 写入 log 文件（dsh-graceful-restart-stderr.log）并转发回终端：
    // 失败时从文件尾部取"具体报错"（而非只有退出码）。
    // 注意：stdio[2] 必须是 'pipe'（'inherit' 时 child.stderr 为 null）；stdout 保持 inherit 继承控制台。
