...
function readSnapshot() {
  try {
    ...
    const history = entries.map((h, i) => {
      if (typeof h.deps === 'object') return { deps: h.deps, at: h.at, note: h.note }
      return { deps: applyDiff(base, h.diff), at: h.at, note: h.note } // diff 条目：相对 history[0] 重建
    })
    return {
      history,
  ...
  try {
    ...
    const history = Array.isArray(s.history) && s.history.length > 0
      ? s.history.map((h, i) => i === 0
        ? { deps: h.deps, at: h.at, note: h.note }
        : { diff: diffDeps(s.history[0].deps, h.deps), at: h.at, note: h.note })
      : []
    const payload = {
      ...s,
      history,
...
function wrapperApply(ctx) {
  ...
  // current（HEAD）携带的错误信息：启动失败时记入，成功时清除
  let currentError = snapshot.current && snapshot.current.error && typeof snapshot.current.error.detail === 'string'
    ? { at: snapshot.current.error.at, detail: snapshot.current.error.detail }
    : null
  ...
  const persist = () => writeSnapshot({
    history,
    current: { deps: current, at: new Date().toISOString(), error: currentError },
    rollbacks,
    rollbackLog,
  })
  persist()
  ...
  // 历史无限保留；但回滚过（head 指针存在）→ 丢弃 head 之前的错误版本
  // （被回滚跳过的基线），再入列新基线。清零回滚计数。
  const markSuccess = () => {
    successTimer = null
    ...
    if (!sameAsHead) {
      // 丢弃 head 之前的错误版本（git：新正确 commit 出现，被回滚跳过的旧版本作废）
      if (rollbackHead !== null && rollbackHead > 0) {
        log('guard: dropping error versions [' + history.slice(0, rollbackHead).map((h) => Object.keys(h.deps).join('|')).join('], [') + ']')
        history = history.slice(rollbackHead)
      }
      // 基线条目携带 note（commit message 式）：回滚后恢复时记录失败/回滚摘要
      let note
      if (rollbackLog && rollbackLog.entries.length > 0) {
        note = '回滚后恢复：' + rollbackLog.entries.map((e) => e.detail).join('；')
      }
      history = [{ deps: current, at: new Date().toISOString(), note }, ...history]
    }
    rollbackHead = null
    rollbacks = 0
    currentError = null // 启动成功：清除 current 上的错误信息
  ...
  const spawnChild = (wake) => {
    ...
    child.once('exit', (code, sig) => {
      ...
      // current = HEAD（期望配置）：失败 → 错误记入 current（版本信息）
      // → current 回退到某基线（HEAD 移动）→ 按差集（回退后的 current vs 实际清单）
      // 调整插件 → 重试；再失败重复（错误覆盖 current，current 再回退）。
      if (failed && history.length > 0 && rollbacks < guardRollbackLimit()) {
        ...
        if (baseline) {
          ...
          // 1) 错误信息记入 current（版本信息）
          const failDetail = '第二代启动失败（退出码 ' + code + '，' + Math.round(elapsed / 1000) + 's）'
          currentError = { at: new Date().toISOString(), detail: failDetail }
          // 2) current 回退（HEAD 移动）
          const rolledBack = baseline
          // 3) 按差集调整插件：回退后的期望(current) vs 实际清单(profile deps)
          const actual = readProfileDeps()
          const diff = rollbackPlugins(actual !== null ? actual : current, rolledBack)
          ...
          // 4) 调整插件后 current = 回退后的期望（与实际一致）；更新快照
          current = rolledBack
          persist()
      ...
      if (failed) {
        ...
        currentError = { at: new Date().toISOString(), detail }
        persist()
