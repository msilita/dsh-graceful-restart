 ...
 * added/removed = 包名增删；changed = 同包名但版本变化（file:/link:/github: 等
 * 路径依赖视为无版本，不参与比较）。
 */
const hasVersion = (v) => typeof v === 'string' && !/^(file:|link:|github:|workspace:|\.|\/)/.test(v)
...
function diffDeps(from, to) {
  ...
  for (const k of Object.keys(to)) {
    if (!(k in from)) {
      added.push(k)
    } else if (hasVersion(from[k]) && hasVersion(to[k]) && from[k] !== to[k]) {
      changed.push(k)
    }
  }
  for (const k of Object.keys(from)) {
    if (!(k in to)) removed.push(k)
  }
  return { added, removed, changed }
}

/** 从完整基准 deps 重建一条差集基线的完整清单（值用占位——仅用于 key 对比/回滚）。 */
function applyDiff(base, diff) {
  const out = { ...base }
  for (const k of diff.removed || []) delete out[k]
  for (const k of diff.added || []) out[k] = '?'
  for (const k of diff.changed || []) out[k] = '?'
  return out
}
 ...
 *   history[0] 存完整 deps（最新基线，基准）；更早基线只存 diff（相对 history[0]）。
 * 读取时重建完整清单；兼容旧版（全 deps 条目 / 单值 ok）。
 */
function readSnapshot() {
  try {
    ...
    const base = entries.length > 0 && typeof entries[0].deps === 'object' ? entries[0].deps : {}
    const history = entries.map((h, i) => {
      if (typeof h.deps === 'object') return { deps: h.deps, at: h.at }
      return { deps: applyDiff(base, h.diff), at: h.at } // diff 条目：相对 history[0] 重建
    })
    return {
      history,
...
function writeSnapshot(s) {
  try {
    // 序列化：history[0] 存完整 deps（基准）；更早基线只存相对 history[0] 的差集
    const history = Array.isArray(s.history) && s.history.length > 0
      ? s.history.map((h, i) => i === 0
        ? { deps: h.deps, at: h.at }
        : { diff: diffDeps(s.history[0].deps, h.deps), at: h.at })
      : []
    const payload = {
      ...s,
      history,
...
function wrapperApply(ctx) {
  ...
  const spawnChild = (wake) => {
    ...
    child.once('exit', (code, sig) => {
      ...
      // 基线选择：优先最新基线（回滚"新增插件"）；若当前与最新基线无差异
      // （失败插件已在上次成功基线内），继续向更早基线回溯（回滚该基线之后
      // 的全部增量）——保证"基线内的插件导致失败"也能被回滚。
      if (failed && history.length > 0 && rollbacks < MAX_ROLLBACKS) {
        let baseline = null
        let baselineIdx = -1
        for (let i = rollbacks; i < history.length; i++) {
          const b = history[i].deps
          if (Object.keys(current).some((k) => !(k in b))) {
            baseline = b
            baselineIdx = i
            break
          }
