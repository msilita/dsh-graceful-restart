...
function applyDiff(base, diff) {
  ...
  for (const a of diff.added || []) out[nameOf(a)] = '?'
  for (const c of diff.changed || []) out[nameOf(c)] = '?'
  return out
}
...
function readSnapshot() {
  try {
    ...
    const history = entries.map((h, i) => {
      ...
      // diff 条目：保留原始 diff（带版本，供视图显示）并重建完整清单
      return { deps: applyDiff(base, h.diff), diff: h.diff, at: h.at, note: h.note, error: h.error }
    })
    return {
      history,
