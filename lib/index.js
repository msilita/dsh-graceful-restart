 ...
 *   history[0] 存完整 deps（最新基线）；更早基线只存相邻差集（相对前一版本，git commit 语义）。
 * 读取时链式重建完整清单（applyDiff 精确）；兼容旧版（全 deps 条目 / 单值 ok）。
 */
  ...
  try {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      if (typeof h.deps === 'object') {
        history.push({ deps: h.deps, at: h.at, note: h.note, error: h.error })
        continue
      }
      // diff 条目：相对"前一版本"（相邻差集）重建，并保留原始 diff（带版本，供视图显示）
      const prev = history.length > 0 ? history[history.length - 1].deps : {}
      history.push({ deps: applyDiff(prev, h.diff), diff: h.diff, at: h.at, note: h.note, error: h.error })
    }
    // 视图统一语义：最新条目（完整 deps）也附加"相对上一版本"的差集（不落盘，仅展示用）
...
function writeSnapshot(s) {
  try {
    // 序列化：history[0] 存完整 deps（最新）；更早基线只存"相邻差集"（相对前一版本，
    // git commit 语义）；无变化条目（与前一版本相同，diff 全空）跳过——不留冗余快照
    const history = Array.isArray(s.history) && s.history.length > 0
      ? s.history.map((h, i) => {
        if (i === 0) return { deps: h.deps, at: h.at, note: h.note, error: h.error }
        const diff = diffDeps(s.history[i - 1].deps, h.deps) // 与前一版本对比
