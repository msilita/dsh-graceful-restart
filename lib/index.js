...
function readSnapshot() {
  try {
    ...
    return {
      ...
      lastError: s.lastError && typeof s.lastError.detail === 'string' ? { at: s.lastError.at, detail: s.lastError.detail } : null,
    }
  } catch { return { history: [], current: null, rollbacks: 0, lastError: null } }
}
...
function wrapperApply(ctx) {
  ...
  // 单槽错误记录：最近一次启动失败/回滚详情，新错误覆盖旧错误（持久化于快照文件）
  let lastError = snapshot.lastError || null
  const recordLastError = (detail) => {
    lastError = { at: new Date().toISOString(), detail }
    log('guard: lastError recorded: ' + detail)
  }
    ...
    setTimeout(async () => {
      ...
      for (const pkg of added) {
        ...
        if (!ok) {
          ...
          recordLastError('新增插件 ' + pkg + ' 客户端加载失败（/plugins/' + pkg + '/client.js 不可用）')
  ...
  const spawnChild = (wake) => {
    ...
    child.once('exit', (code, sig) => {
      ...
      // 基线选择：优先最新基线（回滚"新增/版本变更"）；若当前与最新基线无差异
      ...
      if (failed && history.length > 0 && rollbacks < MAX_ROLLBACKS) {
        ...
        if (baseline) {
          rollbacks++
          const diff = rollbackPlugins(current, baseline)
          const desc = [...diff.added.map((k) => k), ...diff.changed.map((k) => k + '@基线版本')].join(', ')
          const detail = '第二代启动失败（退出码 ' + code + '，' + Math.round(elapsed / 1000)
            + 's），自动回滚卸载：' + (desc || '(无)') + '（基线版本 #' + baselineIdx + '）'
          restoreMarkerForRetry(detail)
          recordLastError(detail)
          console.log('[dsh-graceful-restart] 第二代启动失败（退出码 ' + code + '，' + Math.round(elapsed / 1000)
            + 's），第 ' + rollbacks + ' 次回滚重试（基线版本 #' + baselineIdx + '）...')
          // remove/add 后重新读取清单（以回滚后状态为新基线）并更新快照
