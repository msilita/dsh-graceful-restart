...
async function reloadInPlace(ctx) {
  ...
  for (const entry of loader.entries()) {
    ...
    entries.push({ options, parent: entry.parent, tree: entry.parent && entry.parent.tree })
  }
  log('reload: unloading ' + entries.length + ' entries')
  // 1. 卸载（isDispose=true：保留 options 数据供重建）
  for (const item of entries) {
    try { await item.parent.remove(item.options.id, true) } catch (e) { log('reload: unload ' + item.options.id + ' THREW ' + String(e)) }
  }
  // 2. 清插件模块缓存（非 @deepseek-ai，避免核心包双实例）
  clearPluginCaches(loader)
  // 3. 重建（并发；loader 的 service-availability 调度自动处理依赖顺序）
  log('reload: rebuilding')
  const results = await Promise.allSettled(entries.map((item) => {
    try { return item.tree.create(item.options, item.parent, Infinity) } catch (e) { return Promise.reject(e) }
  }))
  const failed = results.filter((r) => r.status === 'rejected')
  log('reload: rebuilt ok=' + (entries.length - failed.length) + ' failed=' + failed.length)
  return { ok: failed.length === 0, failures: failed.map((f) => String(f.reason && f.reason.message || f.reason)) }
}
