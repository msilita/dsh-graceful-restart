...
function readSnapshot() {
  try {
    ...
    const history = entries.map((h, i) => {
      if (typeof h.deps === 'object') return { deps: h.deps, at: h.at, note: h.note, error: h.error }
      return { deps: applyDiff(base, h.diff), at: h.at, note: h.note, error: h.error } // diff 条目：相对 history[0] 重建
    })
    return {
      history,
  ...
  try {
    ...
    const history = Array.isArray(s.history) && s.history.length > 0
      ? s.history.map((h, i) => i === 0
        ? { deps: h.deps, at: h.at, note: h.note, error: h.error }
        : { diff: diffDeps(s.history[0].deps, h.deps), at: h.at, note: h.note, error: h.error })
      : []
    const payload = {
      ...s,
      history,
...
function wrapperApply(ctx) {
  ...
  const spawnChild = (wake) => {
    ...
    if (latest !== null) {
      current = latest
      // 尝试版本入列（current 指向的版本）：与 history[0] 值比较（版本变化也识别），
      // 与更早条目 key 比较（diff 重建值不精确）；重复不入列
      const keyOf = (deps) => Object.keys(deps).sort().join(',')
      const dup = history.some((h, i) => i === 0
        ? JSON.stringify(h.deps) === JSON.stringify(current)
        : keyOf(h.deps) === keyOf(current))
      if (!dup) history = [{ deps: current, at: new Date().toISOString() }, ...history]
      persist()
