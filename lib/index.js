...
async function wrapperApply(ctx) {
  ...
  // 时机：必须等第一代自身 boot 校验（assertEntriesActivated）完成——
  // 启动阶段拆根会因"条目 fiber 缺失"被判 did-not-activate 导致第一代崩溃（实测）。
  // 当前用 1s 延迟（15:10 轮实证成功组合）；TODO: 改为事件/步骤触发
  // （如轮询全部条目激活后一拍，见 wrapperApply 注释的研究记录）。
  try {
