 ...
 *   启动守护（第一代）：
 *     spawn 前记录插件清单快照（profile package.json dependencies）；
 *     第二代意外退出（退出码非 0，或宽限期 20s 内退出）视为启动失败：
 *       对比"上次成功清单"→ 对新增插件逐个 dsh plugin remove（官方 CLI）→ 重试；
 *       连续失败达上限后保持第一代存活，提示用户手动处理。
 *     第二代存活超过宽限期 → 更新成功基线（并清零回滚计数）。
...
function writeSnapshot(s) {
  try {
    fs.writeFileSync(FILE.snapshot(), JSON.stringify(s, null, 2) + '\n', 'utf8')
...
function wrapperApply(ctx) {
  ...
  // ---- 启动守护：记录插件清单快照 ----
  // 结构：{ ok: {deps, at} | null（上次成功启动的清单）, current: {deps, at}, rollbacks }
  // 第二代启动失败（意外退出/过快退出）时：对比 current vs ok，
  // 对"上次成功之后新增"的插件逐个 dsh plugin remove（官方 CLI），然后重试。
  const profile = profileName()
  const deps = readProfileDeps()
  const snapshot = readSnapshot() || {}
  const okDeps = snapshot.ok && typeof snapshot.ok.deps === 'object' ? snapshot.ok.deps : null
  const okAt = snapshot.ok && typeof snapshot.ok.at === 'string' ? snapshot.ok.at : undefined
  let current = deps !== null
    ? deps
    : (snapshot.current && typeof snapshot.current.deps === 'object' ? snapshot.current.deps : {})
  writeSnapshot({
    ok: snapshot.ok || null,
    current: { deps: current, at: new Date().toISOString() },
    rollbacks: typeof snapshot.rollbacks === 'number' ? snapshot.rollbacks : 0,
  })
  log('guard: current deps [' + Object.keys(current).join(', ') + ']')
  log('guard: ok deps [' + (okDeps ? Object.keys(okDeps).join(', ') : 'none') + ']')
  ...
  let rollbacks = typeof snapshot.rollbacks === 'number' ? snapshot.rollbacks : 0
  ...
  // 第二代存活超过宽限期 → 启动成功 → 更新成功基线（并清零回滚计数）
  const markSuccess = () => {
    successTimer = null
    if (!child || child.exitCode !== null) return
    log('guard: second generation alive past grace, marking success baseline')
    writeSnapshot({
      ok: { deps: current, at: new Date().toISOString() },
      current: { deps: current, at: new Date().toISOString() },
      rollbacks: 0,
    })
  }

  // 卸载"上次成功之后新增"的插件（官方 CLI：node bin.js plugin --profile <p> remove <pkg>）
  const rollbackNewPlugins = (candidate, baseline) => {
    const added = Object.keys(candidate).filter((k) => !(k in baseline))
    if (added.length === 0) return 0
    console.log('[dsh-graceful-restart] 启动失败，自动卸载上次成功后新安装的插件：' + added.join(', '))
    let removed = 0
  ...
  const spawnChild = (wake) => {
    ...
    if (latest !== null) {
      current = latest
      writeSnapshot({
        ok: okDeps ? { deps: okDeps, at: okAt } : null,
        current: { deps: current, at: new Date().toISOString() },
        rollbacks,
      })
    ...
    child.once('exit', (code, sig) => {
      ...
      // 启动失败且存在成功基线、仍有回滚次数 → 卸载新增插件后重试
      if (failed && okDeps && rollbacks < MAX_ROLLBACKS) {
        const added = Object.keys(current).filter((k) => !(k in okDeps))
        if (added.length > 0) {
          rollbacks++
          console.log('[dsh-graceful-restart] 第二代启动失败（退出码 ' + code + '，' + Math.round(elapsed / 1000)
            + 's），第 ' + rollbacks + ' 次回滚重试...')
          rollbackNewPlugins(current, okDeps)
          // remove 后重新读取清单（以回滚后状态为新基线）并更新快照
          const after = readProfileDeps()
          const next = after !== null ? after : okDeps
          current = next
          writeSnapshot({
            ok: okDeps ? { deps: okDeps, at: okAt } : null,
            current: { deps: next, at: new Date().toISOString() },
            rollbacks,
          })
      ...
      if (failed) {
        console.log('[dsh-graceful-restart] 第二代启动失败（退出码 ' + code + '，' + Math.round(elapsed / 1000) + 's），'
          + (okDeps ? '已回滚 ' + rollbacks + ' 次，请手动检查（wrapper 保持运行）' : '且没有可用回滚基线，请手动检查（wrapper 保持运行）'))
