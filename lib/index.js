...
async function wrapperApply(ctx) {
  ...
  // spawn 第二代，随后延迟拆根（15:10 轮实证成功的组合）：
  //   顺序：spawnChild(false)（先启动第二代，服务尽快可用）
  //         → 1s 后 remove('include', true) 拆根（第一代变空壳：零插件零端口）。
  //   时机说明：拆根必须晚于第一代 boot 校验（assertEntriesActivated 判
  //   did-not-activate 崩溃，实测）；1s 延迟是 15:10 轮实证成功的时间点
  //   （boot 校验在 apply 后 <1s 完成）。loader.await()/fiber 轮询均实测过早
  //   （启动早期树未加载完，触发 include 重应用产生损坏条目）。
  //   零插件名、与任何插件零关系。
  spawnChild(false)
    ...
    if (loaderSvc && typeof loaderSvc.remove === 'function') {
      setTimeout(() => {
        loaderSvc.remove('include', true)
          .then(() => log('wrapper: root pruned (first generation is a bare process)'))
