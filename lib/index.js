...
function writeSnapshot(s) {
  try {
    // 序列化：history[0] 存完整 deps（最新快照）+ 相对更早版本的差集（diff 记录时固定）；
    // 更早快照只存"相邻差集"——diff = 该版本相对更早版本（history[i+1]）带来的变化（+ 方向）；
    ...
    const history = Array.isArray(s.history) && s.history.length > 0
      ? s.history.map((h, i) => {
        ...
        if (i === 0) {
          // 最新：完整 deps（基准）+ 固定 diff（记录时算好，读取直接显示不再重算）
          const diff = prev && Object.keys(prev).length > 0 ? diffDeps(prev, h.deps) : undefined
          return { deps: h.deps, at: h.at, note: h.note, ...(diff !== undefined ? { diff } : {}) }
        }
...
/**
 * 活跃会话追踪：记录每个会话最近一次活动时间（agent/status 转换），
 * 用于重启唤醒只针对"最近活跃"的会话——闲置的旧测试会话不应被唤醒
 * （曾出现：闲置会话被唤醒后一直 running 阻塞关闭，还主动 cancel 了关闭安排）。
 */
const ACTIVE_WINDOW_MS = 30 * 60 * 1000 // 30 分钟内活跃过 → 算唤醒目标
const sessionActivity = new Map() // sessionId -> lastActivityAt

/** 追踪会话活跃：agent/status 转换 + 会话事件流（任何事件都算活动）。 */
function trackSessionActivity(ctx) {
  try {
    ...
    // session/event：任何事件（chunk/消息/工具调用等）都刷新活跃时间——
    // 正在跑 LLM 的会话会持续产生事件，不会被误判为闲置
...
/** 最近活跃过的根会话 id（唤醒目标）。 */
function activeRootIds(ctx) {
  const cutoff = Date.now() - ACTIVE_WINDOW_MS
  return ctx.agents.roots()
    .filter((a) => (sessionActivity.get(a.id) || 0) >= cutoff)
    .map((a) => a.id)
}
...
function checkIdleAndExit(ctx) {
  ...
  // 只记录"最近活跃"的根会话（30 分钟窗口）——闲置的旧测试会话不应被唤醒
  // （曾出现：闲置会话被唤醒后一直 running 阻塞关闭，还主动 cancel 了关闭安排）。
  if (pendingAction === 'restart-wake') {
    try {
      const ids = activeRootIds(ctx)
      // 发起者（触发重启的会话）必定在唤醒目标中——它刚安排完重启，必然活跃；
      // 若活跃窗口内未记录（如插件刚启动），显式补上
      if (pendingActionOwner && !ids.includes(pendingActionOwner)) ids.push(pendingActionOwner)
...
function tryResumeSessions(ctx, sessionIds, text, onResult) {
  ...
  const interval = setInterval(() => {
    ...
    if (pending.size === 0) {
      ...
      clearResumeMarker()
      if (typeof onResult === 'function') onResult(false, '会话恢复超时（60s 未恢复）：' + [...pending].join(', '))
    }
  }, 500)
 ...
 *  真正的失败检测由 boot 输出扫描承担（detectBootFailure）——
 *  client/插件加载失败时 dsh 会在启动早期打印 "Failed to load plugins"
 *  / "failed to import loader entry" 等**真实错误输出**，检测错误本身
 *  比静态猜测（模块包装写法、压缩、混淆）全面得多。
 *  返回 null = 就绪；否则返回失败原因。 */
...
function wrapperApply(ctx) {
  ...
  // 同因不再重试保护：上次失败原因（failDetail）——相同原因连续失败时不重复回滚，
  // 避免"误判/环境问题 → 回滚 → 重试 → 又失败"的无限循环（曾出现：client 探测
  // 误报导致反复卸载/装回 dsh-waker-trigger ↔ @my/wake-trigger）。
  ...
  //   added   实际有、目标没有 → 卸载
  //   changed 版本/地址不同 → 卸载后重装目标版本（精确回到目标快照）
  //   removed 实际没有、目标有 → 装回目标版本
  const rollbackPlugins = (candidate, baseline) => {
    ...
    if (added.length === 0 && changed.length === 0 && removed.length === 0) return { added: [], changed: [], removed: [] }
    console.log('[dsh-graceful-restart] 启动失败，自动回滚该快照之后的变更：'
      + '卸载 ' + (added.join(', ') || '(无)') + (changed.length ? '；版本回滚 ' + changed.join(', ') : '')
      + (removed.length ? '；装回 ' + removed.join(', ') : ''))
    const runCli = (args) => {
      try {
    ...
    for (const pkg of changed) {
      ...
      if (runCli(['remove', pkg]) && runCli(['add', addSpec(pkg, spec)])) log('guard: version restored ' + pkg)
      else log('guard: version rollback ' + pkg + ' FAILED')
    }
    for (const pkg of removed) {
      const spec = baseline[pkg]
      log('guard: re-adding ' + pkg + ' → ' + spec)
      if (runCli(['add', addSpec(pkg, spec)])) log('guard: re-added ' + pkg)
      else log('guard: re-add ' + pkg + ' FAILED')
    }
  ...
  const scheduleClientProbe = (childRef) => {
    ...
    setTimeout(async () => {
      ...
      for (const pkg of added) {
        let reason = null
        for (let i = 0; i < 3 && reason !== null; i++) {
          try {
            reason = await probeClientBundle(pkg)
            if (reason) log('guard: probe ' + pkg + ' ' + reason + ' (attempt ' + (i + 1) + ')')
          } catch (e) {
            reason = 'net error: ' + String(e && e.message || e)
            log('guard: probe ' + pkg + ' net error (attempt ' + (i + 1) + '): ' + reason)
          }
          if (reason) await new Promise((r) => setTimeout(r, 1000))
        }
        if (reason) {
          const failDetail = '新增插件 ' + pkg + ' 客户端加载失败（/plugins/' + pkg + '/client.js：' + reason + '）'
          log('guard: client probe FAILED for ' + pkg + ' — treating as startup failure')
          console.log('[dsh-graceful-restart] ' + failDetail + '，视为启动失败，自动回滚...')
          restoreMarkerForRetry(failDetail)
          logRollback(failDetail + '，开始回滚', true)
          errors = [{ at: new Date().toISOString(), detail: failDetail }]
          persist()
  ...
  const spawnChild = (wake, rollbackRetry = false) => {
    ...
    // 回滚重试（rollbackRetry=true）：**不建新快照、不移动 current**——
    // 回滚的语义是"回到某个成功基线并验证它"，成功后 current 就停留在该基线；
    // 快照链只在正常启动（用户主动变更清单）时更新。若逆操作后实际清单与基线
    // 仍有差异（如包名漂移），保持 current 不动，由后续正常启动再对齐。
    newStderrFile() // 新代际：建立独立 stderr 文件（失败时读到的只是本次启动的输出，不累积）
    const latest = readProfileDeps()
    if (latest !== null) {
      if (rollbackRetry) {
        // 清理比 current 新的失败尝试条目（保持链上状态唯一），但不新建、不移动
        const curIdx = history.findIndex((h) => h.at === current)
        if (curIdx > 0) {
          log('guard: rollback retry, discarding ' + curIdx + ' snapshot(s) newer than current ' + current)
          history = history.slice(curIdx)
        }
        lastAdded = [] // 回滚重试不探测新增插件（逆操作已卸载）
        persist()
        log('guard: rollback retry (current kept ' + current + ', deps ['
    ...
    child.once('exit', (code, sig) => {
      ...
      if (failed) {
        ...
        // 同因不再重试：与上一次失败原因相同 → 说明回滚无法改变结果（环境/探测问题），
        // 停止自动回滚，保持进程运行并提示。真实退出码失败每次 code 不同（且会走回滚
        // 逆操作改变清单），不受此保护影响。
        const sameCause = lastFailDetail !== null && failDetail === lastFailDetail
        lastFailDetail = failDetail
        if (sameCause) {
          log('guard: same failure cause repeated, stopping auto-rollback to avoid loop')
          console.log('[dsh-graceful-restart] 检测到与上次相同的失败原因（' + failDetail.slice(0, 120) + '），'
            + '停止自动回滚（避免反复卸载/装回循环）。进程保持运行，请手动检查（可 /shutdown 后重试）。')
          persist()
          return
        }
        // current 指向的快照在 history 中的位置（最新在前；向更早 = 索引 +1）
        const curIdx = history.findIndex((h) => h.at === current)
        const prevIdx = curIdx + 1
        if (curIdx >= 0 && prevIdx < history.length) {
          // 有更早快照 → current 向后（更早）移一条；快照链不修改
          rollbacks++
          current = history[prevIdx].at
          // 差集逆操作：把实际清单恢复到新 current 指向的快照（撤销本次尝试的变更）
...
export function apply(ctx) {
  ...
  // 连接就绪自检（带重试：webServer 监听可能晚于 apply；3 次 × 5s）：
  // 通过 → 唤醒场景开始会话恢复 + steer，非唤醒直接 refresh-ok；
  // 连续失败 → refresh-failed（第一代回滚）。
  const runConnectionCheck = async (source, attempt = 1) => {
    if (refreshReported) return
    const reason = await probeConnectionReady()
    if (reason === null) {
      // 连接层就绪 → 再检查所有 client 半插件的 bundle（页面级失败检测）
      const clients = clientHalfPackages(profileDir())
      let clientReason = null
      for (const pkg of clients) {
        try {
          clientReason = await probeClientBundle(pkg)
          if (clientReason) {
            log('apply: [' + source + '] client bundle check FAILED for ' + pkg + ': ' + clientReason)
            break
          }
        } catch (e) {
          clientReason = 'net error: ' + String(e && e.message || e)
          log('apply: [' + source + '] client bundle check net error for ' + pkg + ': ' + clientReason)
          break
        }
      }
      if (clientReason) {
        reportRefresh(false, '客户端加载失败（' + clientReason + '）')
        return
      }
      log('apply: [' + source + '] connection check OK (attempt ' + attempt + ')')
      refreshLocalOk = true
      if (wake && marker.sessionIds.length > 0) {
        // 唤醒场景：连接就绪 → 会话恢复 + steer；恢复完成/超时决定最终 refresh 结果
    ...
    log('apply: [' + source + '] connection check attempt ' + attempt + ' failed: ' + reason)
    if (attempt < 3) {
      setTimeout(() => runConnectionCheck(source, attempt + 1), 5000)
    } else {
      reportRefresh(false, reason)
    }
  }
  if (wake) {
    ...
    // 15s 兜底：第一代探测逻辑故障时不阻塞唤醒（但仍等刷新自检通过——刷新失败先不唤醒）
    let gated = false
    const gate = () => {
      if (gated) return
      gated = true
    ...
    setTimeout(gate, 15000)
    log('apply: wake gated on probe-ok + refresh self-check (15s fallback)')
  } else {
    // 非唤醒（正常启动 / WAKE=0）：同样做刷新自检（连接就绪），失败同样触发回滚
    log('apply: not a wake launch, refresh self-check still scheduled (startup)')
