...
async function wrapperApply(ctx) {
  ...
  // 拆树（第一代变空壳）：由"第一代自身启动成功"触发——轮询等待所有条目激活
  // （= 第一代 boot 的 await 阶段结束，assertEntriesActivated 校验随即同步执行），
  // 让出一拍（setImmediate）确保校验先完成，再拆树。比 refresh-ok（第二代 5s 自检）
  // 早得多，且确定性基于第一代自身状态，不依赖延迟猜测。
  // 启动阶段拆树会被 assertEntriesActivated 判 did-not-activate 导致第一代崩溃（实测）。
  // 只拆 include 的子条目（include 根是配置树写回载体，拆它会写坏磁盘配置）。
  // 拆树链为原生 async（fiber dispose 不影响链）；失败时不拆（保留现场供回滚诊断）。
  const loaderSvc = ctx.get('loader')
  const pruneTree = () => {
    if (!loaderSvc || typeof loaderSvc.entries !== 'function' || typeof loaderSvc.remove !== 'function') return
    ;(async () => {
      try {
        const entries = [...loaderSvc.entries()].filter((e) => e && e.id !== undefined && String(e.id) !== 'include')
        for (const e of entries) {
          try { await loaderSvc.remove(e.id, true); log('wrapper: pruned entry ' + e.id) }
          catch (err) { log('wrapper: prune ' + e.id + ' SKIP ' + String(err && err.message || err)) }
        }
        log('wrapper: tree pruned (first generation is a bare process)')
      } catch (e) { log('wrapper: pruneTree THREW ' + String(e)) }
    })()
  }

  // 启动成功检测 + 拆树：等所有条目 fiber 激活（自身 apply 已返回，不会死锁），
  // 结构条目（无 fiber）与 disabled 条目跳过。
  const schedulePruneAfterBoot = () => {
    if (!loaderSvc || typeof loaderSvc.entries !== 'function') return
    ;(async () => {
      try {
        for (;;) {
          const all = [...loaderSvc.entries()]
          const activating = all.filter((e) => e && e.fiber && e.fiber.state === 'pending')
          if (activating.length === 0) break
          await new Promise((r) => setTimeout(r, 100))
        }
        // 全部激活 → boot 校验（同步）先完成，再拆树
        await new Promise((r) => setImmediate(r))
        log('wrapper: first generation boot complete, pruning tree')
        pruneTree()
      } catch (e) { log('wrapper: boot-wait THREW ' + String(e)) }
    })()
  }
